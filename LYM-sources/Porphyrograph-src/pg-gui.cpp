...
//////////////////////////////////////////////////////////////////////////////////////
// OSC-BASED TOUCHOSC INTERFACE FEEDBACK
...
void pg_displaySceneVariables(void) {
	if (pg_resend_all_variables) {
		...
		if (pg_FullScenarioActiveVars[pg_ind_scenario][_clipCaptFreq]) {
			...
			sprintf(pg_AuxString, "/clip_autoplay_left %d", int(pg_all_clip_status[pg_enum_clipLeft].get_clip_autoplay(0)));
			pg_send_message_udp((char*)"f", pg_AuxString, (char*)"udp_TouchOSC_send");
			sprintf(pg_AuxString, "/clip_play_left %d", int(pg_all_clip_status[pg_enum_clipLeft].get_clip_play(0)));
			pg_send_message_udp((char*)"f", pg_AuxString, (char*)"udp_TouchOSC_send");
			sprintf(pg_AuxString, "/clip_autoplay_right %d", int(pg_all_clip_status[pg_enum_clipLeft].get_clip_autoplay(0)));
			pg_send_message_udp((char*)"f", pg_AuxString, (char*)"udp_TouchOSC_send");
		...
		for (int pathNo = 1; pathNo < PG_NB_PATHS; pathNo++) {
			...
			sprintf(pg_AuxString, "/path_record_%d %d", pathNo, path_record[pathNo]);
			pg_send_message_udp((char*)"i", pg_AuxString, (char*)"udp_TouchOSC_send");
		}
	}
