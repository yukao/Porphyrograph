...
ScenarioValue InitialValuesInterpVar[_MaxInterpVarIDs][PG_MAX_CONFIGURATIONS];
// rank of the scenario variable in the list of variables generated by python
int ConfigScenarioVarRank[_MaxInterpVarIDs][PG_MAX_CONFIGURATIONS];
// var IDs in scenarios in the order in which they appear in the scenario
string ConfigScenarioVarIDs[_MaxInterpVarIDs][PG_MAX_CONFIGURATIONS];
// number of vars in the scenario file
int ScenarioVarNb[PG_MAX_CONFIGURATIONS] = {0};
// last value shipped to the GUI (PD)
float LastGUIShippedValuesInterpVar[_MaxInterpVarIDs][PG_MAX_CONFIGURATIONS];
...
void pg_initializeScenearioVariables(void) {
	...
	for (int indConfiguration = 0; indConfiguration < pg_NbConfigurations; indConfiguration++) {
		...
		for (int indP = 0; indP < ScenarioVarNb[indConfiguration]; indP++) {
			int indVar = ConfigScenarioVarRank[indP][indConfiguration];
			if (pg_ScenarioActiveVars[indVar][indConfiguration]) {
				if (ScenarioVarTypes[indVar] == _pg_float) {
					*((float*)ScenarioVarPointers[indVar]) = float(InitialValuesInterpVar[indVar][indConfiguration].val_num);
				}
				else if (ScenarioVarTypes[indVar] == _pg_int) {
					*((int*)ScenarioVarPointers[indVar]) = int(InitialValuesInterpVar[indVar][indConfiguration].val_num);
				}
				else if (ScenarioVarTypes[indVar] == _pg_bool) {
					*((bool*)ScenarioVarPointers[indVar]) = (InitialValuesInterpVar[indVar][indConfiguration].val_num != 0);
				}
				else if (ScenarioVarTypes[indVar] == _pg_sign) {
					*((int*)ScenarioVarPointers[indVar]) = (InitialValuesInterpVar[indVar][indConfiguration].val_num > 0 ? 1 : (InitialValuesInterpVar[indVar][indConfiguration].val_num < 0 ? -1 : 0));
				}
				else if (ScenarioVarTypes[indVar] == _pg_path) {
					*((bool*)ScenarioVarPointers[indVar]) = (InitialValuesInterpVar[indVar][indConfiguration].val_num > 0);
				}
				else if (ScenarioVarTypes[indVar] == _pg_string) {
					*((string*)ScenarioVarPointers[indVar]) = InitialValuesInterpVar[indVar][indConfiguration].val_string;
				}
				else {
	...
	if (pg_current_configuration_rank >= 0 && pg_current_configuration_rank < unsigned int(pg_NbConfigurations)) {
		...
		if (pg_ScenarioActiveVars[_path_replay_trackNo_2][pg_current_configuration_rank]) {
		...
		if (pg_ScenarioActiveVars[_path_replay_trackNo_3][pg_current_configuration_rank]) {
		...
		if (pg_ScenarioActiveVars[_path_replay_trackNo_4][pg_current_configuration_rank]) {
		...
		if (pg_ScenarioActiveVars[_path_replay_trackNo_5][pg_current_configuration_rank]) {
		...
		if (pg_ScenarioActiveVars[_path_replay_trackNo_6][pg_current_configuration_rank]) {
		...
		if (pg_ScenarioActiveVars[_path_replay_trackNo_7][pg_current_configuration_rank]) {
		...
		if (pg_ScenarioActiveVars[_path_replay_trackNo_8][pg_current_configuration_rank]) {
		...
		if (pg_ScenarioActiveVars[_path_replay_trackNo_9][pg_current_configuration_rank]) {
		...
		if (pg_ScenarioActiveVars[_path_replay_trackNo_10][pg_current_configuration_rank]) {
		...
		if (pg_ScenarioActiveVars[_path_replay_trackNo_11][pg_current_configuration_rank]) {
		...
		if (pg_ScenarioActiveVars[_path_record_1][pg_current_configuration_rank]) {
			if (!path_record_1) {
		...
		if (pg_ScenarioActiveVars[_path_record_2][pg_current_configuration_rank]) {
			if (!path_record_2) {
		...
		if (pg_ScenarioActiveVars[_path_record_3][pg_current_configuration_rank]) {
			if (!path_record_3) {
		...
		if (pg_ScenarioActiveVars[_path_record_4][pg_current_configuration_rank]) {
			if (!path_record_4) {
		...
		if (pg_ScenarioActiveVars[_path_record_5][pg_current_configuration_rank]) {
			if (!path_record_5) {
		...
		if (pg_ScenarioActiveVars[_path_record_6][pg_current_configuration_rank]) {
			if (!path_record_6) {
		...
		if (pg_ScenarioActiveVars[_path_record_7][pg_current_configuration_rank]) {
			if (!path_record_7) {
		...
		if (pg_ScenarioActiveVars[_path_record_8][pg_current_configuration_rank]) {
			if (!path_record_8) {
		...
		if (pg_ScenarioActiveVars[_path_record_9][pg_current_configuration_rank]) {
			if (!path_record_9) {
		...
		if (pg_ScenarioActiveVars[_path_record_10][pg_current_configuration_rank]) {
			if (!path_record_10) {
		...
		if (pg_ScenarioActiveVars[_path_record_11][pg_current_configuration_rank]) {
			if (!path_record_11) {
...
void pg_displaySceneVariables(void) {
	if (resend_all_variables) {
		...
		if (pg_ScenarioActiveVars[_path_replay_trackNo_1][pg_current_configuration_rank]
			&& pg_ScenarioActiveVars[_path_record_1][pg_current_configuration_rank]) {
		...
		if (pg_ScenarioActiveVars[_path_replay_trackNo_2][pg_current_configuration_rank]
			&& pg_ScenarioActiveVars[_path_record_2][pg_current_configuration_rank]) {
		...
		if (pg_ScenarioActiveVars[_path_replay_trackNo_3][pg_current_configuration_rank]
			&& pg_ScenarioActiveVars[_path_record_3][pg_current_configuration_rank]) {
		...
		if (pg_ScenarioActiveVars[_path_replay_trackNo_4][pg_current_configuration_rank]
			&& pg_ScenarioActiveVars[_path_record_4][pg_current_configuration_rank]) {
		...
		if (pg_ScenarioActiveVars[_path_replay_trackNo_5][pg_current_configuration_rank]
			&& pg_ScenarioActiveVars[_path_record_5][pg_current_configuration_rank]) {
		...
		if (pg_ScenarioActiveVars[_path_replay_trackNo_6][pg_current_configuration_rank]
			&& pg_ScenarioActiveVars[_path_record_6][pg_current_configuration_rank]) {
		...
		if (pg_ScenarioActiveVars[_path_replay_trackNo_7][pg_current_configuration_rank]
			&& pg_ScenarioActiveVars[_path_record_7][pg_current_configuration_rank]) {
		...
		if (pg_ScenarioActiveVars[_path_replay_trackNo_8][pg_current_configuration_rank]
			&& pg_ScenarioActiveVars[_path_record_8][pg_current_configuration_rank]) {
		...
		if (pg_ScenarioActiveVars[_path_replay_trackNo_9][pg_current_configuration_rank]
			&& pg_ScenarioActiveVars[_path_record_9][pg_current_configuration_rank]) {
		...
		if (pg_ScenarioActiveVars[_path_replay_trackNo_10][pg_current_configuration_rank]
			&& pg_ScenarioActiveVars[_path_record_10][pg_current_configuration_rank]) {
		...
		if (pg_ScenarioActiveVars[_path_replay_trackNo_11][pg_current_configuration_rank]
			&& pg_ScenarioActiveVars[_path_record_11][pg_current_configuration_rank]) {
...
void part_move_init(void) {
	if (pg_ScenarioActiveVars[_part_initialization][pg_current_configuration_rank]) {
		...
		if (pg_ScenarioActiveVars[_part_path_follow_0][pg_current_configuration_rank]
			&& pg_ScenarioActiveVars[_part_path_repulse_0][pg_current_configuration_rank]) {
		...
		if (pg_ScenarioActiveVars[_part_path_follow_1][pg_current_configuration_rank]
			&& pg_ScenarioActiveVars[_part_path_repulse_1][pg_current_configuration_rank]) {
		...
		if (pg_ScenarioActiveVars[_part_path_follow_2][pg_current_configuration_rank]
			&& pg_ScenarioActiveVars[_part_path_repulse_2][pg_current_configuration_rank]) {
		...
		if (pg_ScenarioActiveVars[_part_path_follow_3][pg_current_configuration_rank]
			&& pg_ScenarioActiveVars[_part_path_repulse_3][pg_current_configuration_rank]) {
		...
		if (pg_ScenarioActiveVars[_part_path_follow_4][pg_current_configuration_rank]
			&& pg_ScenarioActiveVars[_part_path_repulse_4][pg_current_configuration_rank]) {
		...
		if (pg_ScenarioActiveVars[_part_path_follow_5][pg_current_configuration_rank]
			&& pg_ScenarioActiveVars[_part_path_repulse_5][pg_current_configuration_rank]) {
		...
		if (pg_ScenarioActiveVars[_part_path_follow_6][pg_current_configuration_rank]
			&& pg_ScenarioActiveVars[_part_path_repulse_6][pg_current_configuration_rank]) {
		...
		if (pg_ScenarioActiveVars[_part_path_follow_7][pg_current_configuration_rank]
			&& pg_ScenarioActiveVars[_part_path_repulse_7][pg_current_configuration_rank]) {
		...
		if (pg_ScenarioActiveVars[_part_path_follow_8][pg_current_configuration_rank]
			&& pg_ScenarioActiveVars[_part_path_repulse_8][pg_current_configuration_rank]) {
		...
		if (pg_ScenarioActiveVars[_part_path_follow_9][pg_current_configuration_rank]
			&& pg_ScenarioActiveVars[_part_path_repulse_9][pg_current_configuration_rank]) {
		...
		if (pg_ScenarioActiveVars[_part_path_follow_10][pg_current_configuration_rank]
			&& pg_ScenarioActiveVars[_part_path_repulse_10][pg_current_configuration_rank]) {
		...
		* ((bool*)ScenarioVarPointers[_part_path_follow_11]) = false;
		if (pg_ScenarioActiveVars[_part_path_follow_11][pg_current_configuration_rank]
			&& pg_ScenarioActiveVars[_part_path_repulse_11][pg_current_configuration_rank]) {
...
void part_path_repulse_callBack(int pathNo, pg_Parameter_Input_Type param_input_type, bool scenario_or_gui_command_value) {
	...
	if (pg_ScenarioActiveVars[_part_initialization][pg_current_configuration_rank]) {
		...
		switch (pathNo) {
		case 0:
			if (pg_ScenarioActiveVars[_part_path_repulse_0][pg_current_configuration_rank]) {
				state = part_path_repulse_0;
			}
			break;
#if defined(var_part_path_repulse_1)
		case 1:
			if (pg_ScenarioActiveVars[_part_path_repulse_1][pg_current_configuration_rank]) {
		...
		case 2:
			if (pg_ScenarioActiveVars[_part_path_repulse_2][pg_current_configuration_rank]) {
		...
		case 3:
			if (pg_ScenarioActiveVars[_part_path_repulse_3][pg_current_configuration_rank]) {
		...
		case 4:
			if (pg_ScenarioActiveVars[_part_path_repulse_4][pg_current_configuration_rank]) {
		...
		case 5:
			if (pg_ScenarioActiveVars[_part_path_repulse_5][pg_current_configuration_rank]) {
		...
		case 6:
			if (pg_ScenarioActiveVars[_part_path_repulse_6][pg_current_configuration_rank]) {
		...
		case 7:
			if (pg_ScenarioActiveVars[_part_path_repulse_7][pg_current_configuration_rank]) {
		...
		case 8:
			if (pg_ScenarioActiveVars[_part_path_repulse_8][pg_current_configuration_rank]) {
		...
		case 9:
			if (pg_ScenarioActiveVars[_part_path_repulse_9][pg_current_configuration_rank]) {
		...
		case 10:
			if (pg_ScenarioActiveVars[_part_path_repulse_10][pg_current_configuration_rank]) {
		...
		case 11:
			if (pg_ScenarioActiveVars[_part_path_repulse_11][pg_current_configuration_rank]) {
...
void pg_update_variable(pg_Parameter_Input_Type param_input_type, int indVar,
	...
	pg_variable_updated[indVar] = true;
	pg_variable_param_input_type[indVar] = param_input_type;
	pg_variable_scenario_or_gui_command_value[indVar] = scenario_or_gui_command_value;
...
void pg_run_a_callBack(int indVar) {
	if (pg_variable_updated[indVar] && ScenarioVarCallbacks[indVar]) {
		//printf("Run callback of var \"%s\"\n", ScenarioVarMessages[indVar]);
		(*ScenarioVarCallbacks[indVar])(pg_variable_param_input_type[indVar], pg_variable_scenario_or_gui_command_value[indVar]);
	}
			...
			if ((transformedInterpolation == 0.0 && pg_FirstFrameInScene)
				...
				if (cur_scene->scene_interpolations[indVar].interpolation_mode == pg_stepwise_interpolation) {
					...
					if (transformedInterpolation > 0.0
						&& StepwiseInterpolationEffective[indVar] == false) {
						pg_update_variable(_PG_SCENARIO, indVar, cur_scene->scene_final_parameters[indVar]);
					...
					// initial stepwise value
						...
						pg_update_variable(_PG_SCENARIO, indVar, cur_scene->scene_initial_parameters[indVar]);
...
void ReceiveBeat(void) {
	...
	if (pg_ScenarioActiveVars[_path_record_1][pg_current_configuration_rank]
		&& pg_ScenarioActiveVars[_path_replay_trackNo_1][pg_current_configuration_rank]) {
		// loop start 
...
void pg_aliasScript(string address_string, string string_argument_0,
	...
	if (pg_OSC_addresses_hashMap[address_string] < _MaxInterpVarIDs) {
		if (pg_ScenarioActiveVars[pg_OSC_addresses_hashMap[address_string]][pg_current_configuration_rank]) {
			...
			pg_update_variable(_PG_GUI_COMMAND, pg_OSC_addresses_hashMap[address_string], arg_val);
		}
		return;
	}
