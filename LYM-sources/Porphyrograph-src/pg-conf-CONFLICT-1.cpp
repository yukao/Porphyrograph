...
void parseConfigurationFile(std::ifstream& confFin, std::ifstream&  scenarioFin) {
	...
	for (int indScene = 0; indScene < pg_NbScenes; indScene++) {
		...
		for (int indP = 0; indP < _MaxInterpVarIDs; indP++) {
			...
			sstream >> temp;
			bool has_only_digits = (temp.find_first_not_of("0123456789-.") == string::npos);
			if (!has_only_digits && temp.compare("*") != 0) {
				sprintf(ErrorStr, "Error: non numeric variable initial value in scene %d var %d (%s)\n", indScene + 1, indP, temp.c_str()); ReportError(ErrorStr); throw 50;
			}
			else if (temp.compare("*") == 0) {
				pg_Scenario[indScene].scene_initial_parameters_is_current_value[indP] = true;
				pg_Scenario[indScene].scene_initial_parameters[indP] = 0.;
			}
			else {
				pg_Scenario[indScene].scene_initial_parameters_is_current_value[indP] = false;
		...
		for (int indP = 0; indP < _MaxInterpVarIDs; indP++) {
			...
			case 'S':
				...
				if (valCh == 'S') {
					...
					float val = stringToDuration(vals, pg_Scenario[indScene].scene_duration, indScene + 1, indP + 1);
					if (val < 0.0) {
						sprintf(ErrorStr, "Error: offset values of S(tepwise) in scene %d var %d lower than 0.0: %.3f\n", indScene + 1, indP + 1, val); ReportError(ErrorStr); throw 50;
					}
					if (val <= 1.0) {
						pg_Scenario[indScene].scene_interpolations[indP].offSet = val;
						...
						sprintf(ErrorStr, "Error: offset value of S(tepwise) in scene %d var %d greater than 1.0: %.3f\n", indScene + 1, indP + 1, val); ReportError(ErrorStr); throw 50;
					}
				}
				break;
			case 'k':
			case 'K':
				...
				break;
			default:
				sprintf(ErrorStr, "Error: unknown  interpolation mode in scene %d parameter %d [%d] [%c]!", indScene + 1, indP + 1, int(valCh), valCh); ReportError(ErrorStr); throw 50;
				break;
			}
