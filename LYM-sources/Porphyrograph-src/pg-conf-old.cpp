...
int                      nb_paths[_NbConfigurations] = { 0 };
path*				 paths[_NbConfigurations] = { NULL };
...
void ParseScenarioClipsAndPhotos(std::ifstream& scenarioFin, int indConfiguration) {
	...
	if (nb_clip_albums[indConfiguration] > 0) {
		pg_ClipDirectory = clipAlbumDirName[indConfiguration];
	}
	else {
		pg_ClipDirectory = "";
	}
...
void ParseScenarioClipArtPaths(std::ifstream& scenarioFin, int indConfiguration) {
	std::stringstream  sstream;
	string line;
	string ID;
	string temp;
	...
	nb_paths[indConfiguration] = 0;
	current_path_group = 0;
	sstream >> nb_paths[indConfiguration];
	//printf("nb svg paths %d\n", nb_paths[indConfiguration]);
	paths[indConfiguration] = new path[nb_paths[indConfiguration]]();

	for (int indPrerecPath = 0; indPrerecPath < nb_paths[indConfiguration]; indPrerecPath++) {
		std::getline(scenarioFin, line);
		stringstreamStoreLine(&sstream, &line);
		sstream >> ID; // string svg_path
		sstream >> temp; // file name
		string fileName = "Data/" + project_name + "-data/ClipArt/" + temp;
		sstream >> temp2;
		...
		printf("Loading path ID [%s] %s\n", path_ID.c_str(), fileName.c_str());
		//printf("path no %d group %d\n", paths[indConfiguration][indPrerecPath].indPath, paths[indConfiguration][indPrerecPath].path_group);
		paths[indConfiguration][indPrerecPath].ClipArt_path_init(indPath, indTrack, pathRadius, path_r_color, path_g_color, path_b_color, path_readSpeedScale,
		...
		for (int indAux = 0; indAux < indPrerecPath; indAux++) {
			if (paths[indConfiguration][indAux].indPath == paths[indConfiguration][indPrerecPath].indPath && paths[indConfiguration][indAux].path_group == paths[indConfiguration][indPrerecPath].path_group) {
				sprintf(ErrorStr, "Error: incorrect configuration file paths %d and %d have the same path index %d and same path group %d", indAux, indPrerecPath, paths[indConfiguration][indAux].indPath, paths[indConfiguration][indAux].path_group); ReportError(ErrorStr); throw 100;
			}
		}
		...
		if (indTrack >= 0 && indTrack < PG_NB_TRACKS && indPath >= 1 && indPath <= PG_NB_PATHS) {
			...
			sprintf(ErrorStr, "Error: incorrect scenario file track %d for ClipArt path %d number (\"%s\")",
				indTrack, indPath, fileName.c_str()); ReportError(ErrorStr); throw 100;
		}
		//std::cout << "svg_path #" << indPath << ": " << "Data/" + project_name + "-data/ClipArt/" + temp << " track #" << indTrack << "\n";
	}
	// /svg_paths
	std::getline(scenarioFin, line);
	stringstreamStoreLine(&sstream, &line);
...
void ParseScenarioClipArt(std::ifstream& scenarioFin, int indConfiguration) {
	...
	for (int indClipArtFile = 0; indClipArtFile < pg_nb_ClipArt[indConfiguration]; indClipArtFile++) {
		...
		printf("Loading %s\n", pg_ClipArt_fileNames[indConfiguration][indClipArtFile].c_str());
...
void ParseScenarioMeshes(std::ifstream& scenarioFin, int indConfiguration) {
	...
	if (ScenarioVarConfigurations[_activeMeshes][indConfiguration]) {
		...
		Mesh_texture_rectangle[indConfigurations] = new GLuint[pg_nb_Mesh_files];
		memset((char*)Mesh_texture_rectangle[indConfigurations], 0, pg_nb_Mesh_files * sizeof(GLuint));
...
void parseScenarioFile(std::ifstream& scenarioFin, int indConfiguration) {
	...
	ParseScenarioClipArtPaths(scenarioFin, indConfiguration);
