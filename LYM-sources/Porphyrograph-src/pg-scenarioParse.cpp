...
// UTIL: VAR ID TO RANK
int pg_varID_to_rank(string var_ID) {
	for (int indP = 0; indP < _MaxInterpVarIDs; indP++) {
		if (pg_FullScenarioVarStrings[indP] == var_ID) {
			return indP;
		}
	}
	sprintf(pg_errorStr, "Error: scenario variable %s not found in the variable string list generated from full scenario\n", var_ID.c_str()); pg_ReportError(pg_errorStr); throw 50;
	return -1;
}
