...
int pg_SceneAfterInterpolation = -1;
...
std::unordered_map<std::string, int> pg_stringCommands = {
	...
	{ "setup_with_interpolation", _setup_with_interpolation },
#ifdef CRITON
	...
	if (true ) { //pg_FrameNo % 3 == 0 || pg_FirstFrameInScene) {
		...
		if (resend_all_variables) {
			if (pg_CurrentScene >= 0 && pg_CurrentScene < pg_NbScenes) {
				sprintf(AuxString, "/setup %s", pg_Scenario[pg_CurrentScene].scene_IDs.c_str()); pg_send_message_udp((char *)"s", AuxString, (char *)"udp_TouchOSC_send");
				sprintf(AuxString, "/setup_1 %s", pg_Scenario[pg_CurrentScene].scene_Msg1.c_str()); pg_send_message_udp((char *)"s", AuxString, (char *)"udp_TouchOSC_send");
				sprintf(AuxString, "/setup_2 %s", pg_Scenario[pg_CurrentScene].scene_Msg2.c_str()); pg_send_message_udp((char *)"s", AuxString, (char *)"udp_TouchOSC_send");
			}
			else {
				sprintf(AuxString, "/setup initial_setup"); pg_send_message_udp((char *)"s", AuxString, (char *)"udp_TouchOSC_send");
				sprintf(AuxString, "/setup_1 ******************_arreter_AVG_*********************"); pg_send_message_udp((char *)"s", AuxString, (char *)"udp_TouchOSC_send");
				sprintf(AuxString, "/setup_2 ****************_verifier_camera_*******************"); pg_send_message_udp((char *)"s", AuxString, (char *)"udp_TouchOSC_send");
			}
...
#if defined(PIERRES) || defined(ATELIERSENFANTS)
...
void pg_update_visual_and_text_chapters(bool new_scene) {
	...
	if ((pg_CurrentScene > 0 || pg_NbScenes < 13) && (pg_CurrentScene < pg_NbScenes - 1)) {
		...
		if (new_scene) {
			...
			pg_CurrentDiaporamaDir
				= (int(elapsed_time_from_start - pg_Scenario[pg_CurrentScene].scene_initial_time) / 60)
...
void StartNewScene(int ind_scene, float interpolation_duration) {
	...
	if (interpolation_duration <= 0) {
		pg_SceneAfterInterpolation = -1;
		...
		sprintf(AuxString, "/setup_1 %s", pg_Scenario[pg_CurrentScene].scene_Msg1.c_str()); pg_send_message_udp((char*)"s", AuxString, (char*)"udp_TouchOSC_send");
		sprintf(AuxString, "/setup_2 %s", pg_Scenario[pg_CurrentScene].scene_Msg2.c_str()); pg_send_message_udp((char*)"s", AuxString, (char*)"udp_TouchOSC_send");
		std::cout << "Scene: " << pg_Scenario[pg_CurrentScene].scene_IDs << std::endl;
		// fprintf(pg_csv_file, "Scene:¨%d %s\n", pg_CurrentScene, pg_Scenario[pg_CurrentScene].scene_IDs.c_str());
		...
		for (int indVar = 0; indVar < _MaxInterpVarIDs; indVar++) {
			...
			if (pg_Scenario[pg_CurrentScene].scene_initial_parameters_is_current_value[indVar] == true) {
				if (ScenarioVarTypes[indVar] == _pg_float) {
					pg_Scenario[pg_CurrentScene].scene_initial_parameters[indVar] = double(*((float*)ScenarioVarPointers[indVar]));
				}
				else if (ScenarioVarTypes[indVar] == _pg_int) {
					pg_Scenario[pg_CurrentScene].scene_initial_parameters[indVar] = double(*((int*)ScenarioVarPointers[indVar]));
				}
				else if (ScenarioVarTypes[indVar] == _pg_bool) {
					if (*((bool*)ScenarioVarPointers[indVar]) != 0) {
						pg_Scenario[pg_CurrentScene].scene_initial_parameters[indVar] = 1;
					}
					else {
						pg_Scenario[pg_CurrentScene].scene_initial_parameters[indVar] = 0;
					}
				}
				else if (ScenarioVarTypes[indVar] == _pg_sign) {
					pg_Scenario[pg_CurrentScene].scene_initial_parameters[indVar] = double(*((float*)ScenarioVarPointers[indVar]));
	...
	// variables all keep their current values and interpolate towards the values of the variables in the target scene
		...
		pg_SceneAfterInterpolation = ind_scene;

		//to be completed

		pg_InterpolationScene.scene_duration = interpolation_duration;
		pg_InterpolationScene.scene_change_when_ends = true;
		pg_InterpolationScene.scene_initial_time = 0.0;
		pg_InterpolationScene.scene_final_time = 0.0;
		pg_InterpolationScene.scene_originalDuration = 0.0;
		pg_InterpolationScene.scene_originalInitial_time = 0.0;
		pg_InterpolationScene.scene_originalFinal_time = 0.0;

		pg_InterpolationScene.scene_initial_parameters = new double[_MaxInterpVarIDs];
		pg_InterpolationScene.scene_initial_parameters_is_current_value = new bool[_MaxInterpVarIDs];
		pg_InterpolationScene.scene_final_parameters = new double[_MaxInterpVarIDs];
		for (int indP = 0; indP < _MaxInterpVarIDs; indP++) {
			pg_InterpolationScene.scene_initial_parameters[indP] = 0.0;
			pg_InterpolationScene.scene_initial_parameters_is_current_value[indP] = false;
			pg_InterpolationScene.scene_final_parameters[indP] = 0.0;
		}
		pg_InterpolationScene.scene_interpolations = new pg_Interpolation[_MaxInterpVarIDs];
		for (int indP = 0; indP < _MaxInterpVarIDs; indP++) {
			pg_InterpolationScene.scene_interpolations[indP].interpolation_mode = pg_linear_interpolation;
			pg_InterpolationScene.scene_interpolations[indP].offSet = 0.0;
			pg_InterpolationScene.scene_interpolations[indP].duration = 1.0;
		}
	...
	for (int ind_scene = 0; ind_scene < pg_NbScenes; ind_scene++) {
		...
		if( (elapsed_time_from_start >= pg_Scenario[ind_scene].scene_initial_time
			...
			if (ind_scene != pg_CurrentScene) {
				// a new scene is launched only if pg_Scenario[pg_CurrentScene].scene_change_when_ends
				if (pg_Scenario[pg_CurrentScene].scene_change_when_ends) {
					StartNewScene(ind_scene, 0.f);
				...
				// the parameter values stay as they were 
					...
					remainingTimeInScene
						= pg_Scenario[pg_CurrentScene].scene_initial_time + pg_Scenario[pg_CurrentScene].scene_duration - elapsed_time_from_start;
...
void setup(int scene_ind, float interpolation_duration) {
	if (scene_ind >= 0 && pg_CurrentScene < 0) {
		pg_launch_performance(scene_ind % pg_NbScenes);
	}
	else {
		StartNewScene(scene_ind % pg_NbScenes, interpolation_duration);
	}
}
...
void setup_plus(int incay) {
	if (pg_CurrentScene < 0) {
		...
		int new_scene = ((incay + pg_CurrentScene) % pg_NbScenes);
		StartNewScene(new_scene, 0.f);
	}
}

void setup_minus(int decay) {
	int new_scene = ((pg_CurrentScene - decay + pg_NbScenes) % pg_NbScenes);
	StartNewScene(new_scene, 0.f);
}
...
void pg_keyStrokeScripts(int key) {
	...
	case's': {
		restoreInitialTimesAndDurations();
		float deltaTime = pg_Scenario[1 + pg_CurrentScene].scene_initial_time - (CurrentClockTime - InitialScenarioTime);
...
void pg_aliasScript(char *command_symbol, char* string_argument_0,
	...
	case _new_scene: {
		if (pg_CurrentScene < 0) {
			...
			StartNewScene(int(arguments[0]), 0.f);
		}
		break;
	}
...
#define PG_PRESSURE_STANDARD 10.0f
	...
	case _setup: {
		setup(int(round(arguments[0])), 0.f);
		break;
	}
	case _setup_plus: {
		setup_plus(1);
	...
	case _setup_with_interpolation: {
		int newScene = int(round(arguments[0]));
		float interpolation_duration = arguments[1];
		setup(newScene, interpolation_duration);
		break;
	}
	case _setup_plus_5: {
		setup_plus(5);
	...
	case _setup_plus_keep_total_dur: {
		float elapsedTime = (CurrentClockTime - InitialScenarioTime) - pg_Scenario[pg_CurrentScene].scene_initial_time;
		float deltaTime = pg_Scenario[1 + pg_CurrentScene].scene_initial_time - (CurrentClockTime - InitialScenarioTime);
