...
GLint uniform_Master_fs_3fv_Caverne_BackColor_rgb[PG_MAX_CONFIGURATIONS] = {-1};
#endif
#if defined(var_flashchange_BGcolor_freq)
GLint uniform_Master_fs_3fv_BG_color_rgb[PG_MAX_CONFIGURATIONS] = {-1};
#endif
...
// #include "pg_scripts/pg_shader_body_bind.cpp"
	...
	for (int indConfig = 0; indConfig < pg_NbConfigurations; indConfig++) {
		...
		if (pg_ScenarioActiveVars[_part_initialization][indConfig]) {
			////////////////////////////////////////////////////////////////////////////////
		...
		if (pg_ScenarioActiveVars[_nb_CATypes][indConfig]) {
...
#if defined(var_flashPhotoTrkBeat) && defined(var_flashPhotoTrkBright)  && defined(var_flashPhotoTrkLength)  && defined(var_flashPhotoChangeBeat)
		if (pg_ScenarioActiveVars[_flashPhotoTrkBeat][indConfig]
			&& pg_ScenarioActiveVars[_flashPhotoTrkBright][indConfig]
			&& pg_ScenarioActiveVars[_flashPhotoTrkLength][indConfig]
			&& pg_ScenarioActiveVars[_flashPhotoChangeBeat][indConfig]) {
...
#endif
		if (pg_ScenarioActiveVars[_photo_diaporama][indConfig]) {
...
#if defined(var_clipCaptFreq) && PG_NB_PARALLEL_CLIPS >= 2
		if (pg_ScenarioActiveVars[_clipCaptFreq][indConfig]) {
...
#endif
		if (pg_ScenarioActiveVars[_cameraCaptFreq][indConfig]) {
...
#if defined(var_CATable)
		if (pg_ScenarioActiveVars[_CATable][indConfig]) {
...
#endif
#if defined(var_GenerativeNights_planes)
		if (pg_ScenarioActiveVars[_GenerativeNights_planes][indConfig]) {
		...
		if (pg_ScenarioActiveVars[_camera_BG_subtr][indConfig]) {
...
#if defined(var_BG_CA_repop_density)
		if (pg_ScenarioActiveVars[_BG_CA_repop_density][indConfig]) {
...
#if defined(var_Caverne_BackColor)
		if (pg_ScenarioActiveVars[_Caverne_BackColor][indConfig]) {
...
#endif
#if defined(var_flashchange_BGcolor_freq)
		if (pg_ScenarioActiveVars[_flashchange_BGcolor_freq][indConfig]) {
...
#if defined(var_sensor_layout)
		if (pg_ScenarioActiveVars[_sensor_layout][indConfig]) {
			////////////////////////////////////////////////////////////////////////////////
...
#if defined(var_activeMeshes)
		if (pg_ScenarioActiveVars[_activeMeshes][indConfig]) {
			////////////////////////////////////////////////////////////////////////////////
...
#if defined(var_Contact_mesh_expand)
			if (pg_ScenarioActiveVars[_Contact_mesh_expand][indConfig]) {
...
#endif
#if defined(var_Contact_mesh_anime)
			if (pg_ScenarioActiveVars[_Contact_mesh_anime][indConfig]) {
				pg_allocateBindAndCheckUniform(indConfig, uniform_Mesh_bones_matrices, "uniform_Mesh_bones_matrices", _pg_shader_Mesh);
			}
#endif
#if defined(var_MmeShanghai_brokenGlass)
			if (pg_ScenarioActiveVars[_MmeShanghai_brokenGlass][indConfig]) {
			...
			if (pg_ScenarioActiveVars[_textureFrontier_wmin][indConfig]) {
...
#endif
#if defined(var_Contact_mesh_palette)
			if (pg_ScenarioActiveVars[_Contact_mesh_palette][indConfig]) {
