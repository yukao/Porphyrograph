...
STARTUPINFOA pg_vlc_si_script = { 0 };
PROCESS_INFORMATION pg_vlc_pi_script;
#endif
...
void pg_StartNewScene(int ind_scene, double delta_time) {
	...
	if (pg_CurrentScene) {
		...
		if (pg_CurrentScene->scene_IDs == "lumiere0") {
			sprintf(pg_AuxString, "/lightOK %.2f", 1.f); pg_send_message_udp((char*)"f", (char*)pg_AuxString, (char*)"udp_Python_send");
#ifdef _WIN32
			sprintf(pg_AuxString,
				"vlc.exe --fullscreen  --qt-fullscreen-screennumber=1 file:///E:/LYM-videos-sources/LYM_Lavoirs_2024/VideoBethleem_2024_v2.mp4");
			pg_vlc_si_script.cb = sizeof(STARTUPINFO);
			pg_vlc_si_script.dwFlags = STARTF_USESHOWWINDOW;
			pg_vlc_si_script.wShowWindow = TRUE;
			//if (pg_pi_script_1.hProcess != NULL) {
			//	TerminateProcess(pg_pi_script_1.hProcess, 0);
			//}
			if (!CreateProcessA(NULL,   // No module name (use command line)
				pg_AuxString,        // Command line
				...
				&pg_vlc_si_script,            // Pointer to STARTUPINFO structure
				&pg_vlc_pi_script)           // Pointer to PROCESS_INFORMATION structure
				)
			{
		...
		else if (pg_CurrentScene->scene_IDs == "lumiereIn") {
			sprintf(pg_AuxString, "/lightOK %.2f", 0.f); pg_send_message_udp((char*)"f", (char*)pg_AuxString, (char*)"udp_Python_send");
		}
