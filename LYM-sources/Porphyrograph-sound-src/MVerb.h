////////////////////////////////////////////////////////////////////
// Studio quality, open-source reverb. Its release was intended to 
// provide a practical demonstration of Dattorro’s figure-of-eight 
// reverb structure and provide the open source community with a 
// high quality reverb.
//
// All code is self-contained within the mverb.h file and an example 
// showing inclusion in a VST plugin is included.
//
// Binaries for Windows VST, Mac VST and Linux VST can be found 
// at http://rekkerd.org/martin-eastwood-audio-free-plugins/Studio 
// quality, open-source reverb. Its release was intended to provide 
// a practical demonstration of Dattorro’s figure-of-eight reverb 
// structure and provide the open source community with a high 
// quality reverb.
//
// All code is self-contained within the mverb.h file and an example 
// showing inclusion in a VST plugin is included.
//
// Binaries for Windows VST, Mac VST and Linux VST can be found 
// at http://rekkerd.org/martin-eastwood-audio-free-plugins/mda 
// VST plug-ins http://mda.smartelectronix.com/
//
...
public:
	...
	void Init(void) {
	  VST_reverb_samples.setParameter(MVerb<float>::DAMPINGFREQ, 0.f);
	  VST_reverb_samples.setParameter(MVerb<float>::DENSITY, 0.5f);
	  VST_reverb_samples.setParameter(MVerb<float>::BANDWIDTHFREQ, 1.f);
	  VST_reverb_samples.setParameter(MVerb<float>::DECAY, 0.5f);
	  VST_reverb_samples.setParameter(MVerb<float>::PREDELAY, 0.f);
	  VST_reverb_samples.setParameter(MVerb<float>::SIZE, 0.5f);
    ...
    void processReplacing(T **inputs, T **outputs, int sampleFrames){
        T OneOverSampleFrames = 1.f / sampleFrames;
        T MixDelta	= (Mix - MixSmooth) * OneOverSampleFrames;
        T EarlyLateDelta = (EarlyMix - EarlyLateSmooth) * OneOverSampleFrames;
        T BandwidthDelta = (((BandwidthFreq * 18400.f) + 100.f) - BandwidthSmooth) * OneOverSampleFrames;
        T DampingDelta = (((DampingFreq * 18400.f) + 100.f) - DampingSmooth) * OneOverSampleFrames;
    ...
    float getParameter(int index){
        switch(index){
            case DAMPINGFREQ:
                    return DampingFreq * 100.f;
                    break;
            case DENSITY:
                    return Density1 * 100.f;
                    break;
            case BANDWIDTHFREQ:
                    return BandwidthFreq * 100.f;
                    break;
            case PREDELAY:
                    return PreDelayTime * 100.f;
                    break;
            case SIZE:
                    return ((0.95f * Size) + 0.05f)*100.f;
                    break;
            case DECAY:
                    return Decay * 100.f;
                    break;
...
template<typename T, int OverSampleCount>
    ...
    private:
        ...
        {
            f = 2.f * sinf(float(3.141592654 * frequency / sampleRate));
        }
	};
#endif
