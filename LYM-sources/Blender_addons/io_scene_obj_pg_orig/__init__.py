...
bl_info = {
    "name": "Wavefront OBJ format + bone weights for porphyrograph",
    "author": "Campbell Barton, Bastien Montagne, Yukao Nagemi",
    "version": (3, 8, 1),
    "blender": (2, 81, 6),
    "location": "File > Import-Export",
    "description": "Import-Export OBJ, Import OBJ mesh, UV's, materials and textures, vertex bone weights for porphyrograph",
    "warning": "",
...
if "bpy" in locals():
    import importlib
    if "import_obj_pg" in locals():
        importlib.reload(import_obj_pg)
    if "export_obj_pg" in locals():
        importlib.reload(export_obj_pg)
...
class ImportOBJ(bpy.types.Operator, ImportHelper):
    ...
    def execute(self, context):
        ...
        return import_obj_pg.load(context, **keywords)
...
class ExportOBJ(bpy.types.Operator, ExportHelper):
    ...
    use_bones: BoolProperty(
            name="Include Bones",
            description="Export armature for porphyrograph",
            default=True,
            )
    use_smooth_groups: BoolProperty(
            name="Smooth Groups",
    ...
    use_poses: BoolProperty(
            name="Export Poses (root bone translation, all other bones quaternions)",
            description="",
            default=True,
            )
    ...
    def execute(self, context):
        ...
        keywords["global_matrix"] = global_matrix
        return export_obj_pg.save(context, **keywords)
...
class OBJ_PT_export_geometry(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        layout.prop(operator, 'use_bones')
        ...
        col.prop(operator, 'use_poses', text="Export Poses")
        col.prop(operator, "frame_start", text="Frame Start")
        col.prop(operator, "frame_end", text="End")


def menu_func_import(self, context):
    self.layout.operator(ImportOBJ.bl_idname, text="Wavefront for porphyrograph (.obj)")


def menu_func_export(self, context):
    self.layout.operator(ExportOBJ.bl_idname, text="Wavefront for porphyrograph (.obj)")
