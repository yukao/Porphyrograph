  ...
  while( indChar <= (int)strlen( pathString ) && curChar != EOF  && curChar != '"' ) {
	...
	while( _Num(curChar) || curChar == '-' || curChar == '+' ) { 
	  if( relative  && *indSpline != 0 )
	    currentPoint[0] += ReadFloatString( &curChar , true  , 
					 pathString , &indChar );
	  else
	    currentPoint[0] = ReadFloatString( &curChar , true  , 
					  pathString , &indChar );
	  ...
	  if( relative  && *indSpline != 0 )
	    currentPoint[1] += ReadFloatString( &curChar , true  , 
					  pathString , &indChar );
	  else
	    currentPoint[1] = ReadFloatString( &curChar , true  , 
					  pathString , &indChar );
	  ...
	  if( (*indSpline) < max_mouse_recording_frames ) {
	    if( isSource ) {
	      if( (*indSpline) ) { // not first frame -> adds a break in the curve
		...
		TrackPos_x_source[ indTrack ][ (*indSpline) ] = -1000.0;
		TrackPos_y_source[ indTrack ][ (*indSpline) ] = -1000.0;
		...
		TrackPos_x_target[ indTrack ][ (*indSpline) ] = -1000.0;
		TrackPos_y_target[ indTrack ][ (*indSpline) ] = -1000.0;
...
void updateMouseEnvironmentVariablesAndTables( double theTime ) {
  ...
  if( currentTrack >= 0
	...
	= brushRadius + (currentTrack == 0 ? brush_radius_vol * brush_radius_vol_coef:0.0f)
	  + (withTabletPressureRadius ? tabletPressureRadius * brush_radius_pressure_coef: 0.0f)
	  + (withTabletAngleRadius ? fabs( sin(tabletAzimutRadius) )  * tabletInclinationRadius * brush_radius_angleHor_coef: 0.0f);
      TrackRadiusY_source[ currentTrack ][ indFrameRec ] 
	= brushRadius + (currentTrack == 0 ? brush_radius_vol * brush_radius_vol_coef:0.0f)
	  + (withTabletPressureRadius ? tabletPressureRadius * brush_radius_pressure_coef: 0.0f)
	  + (withTabletAngleRadius ? fabs( cos(tabletAzimutRadius) ) * tabletInclinationRadius * brush_radius_angleVer_coef: 0.0f);
#else
      TrackRadiusX_source[ currentTrack ][ indFrameRec ] 
	= brushRadius+ (currentTrack == 0 ? brush_radius_vol * brush_radius_vol_coef:0.0f);
      TrackRadiusY_source[ currentTrack ][ indFrameRec ] 
  ...
  for( int indTrack = 0 ; 
	...
	if( TrackPos_x_source[ indTrack ][ TrackStatus_source[ indTrack ].indReading ] < 0 
	    ...
	    || ( TrackStatus_source[ indTrack ].indReading > 0 
		  && fabs(TrackPos_x_source[ indTrack ][ TrackStatus_source[ indTrack ].indReading - 1 ] 
	    	      - TrackPos_x_source[ indTrack ][ TrackStatus_source[ indTrack ].indReading ]) > 100 )
	    || ( TrackStatus_source[ indTrack ].indReading > 0 
		  && fabs(TrackPos_y_source[ indTrack ][ TrackStatus_source[ indTrack ].indReading - 1 ] 
	    	      - TrackPos_y_source[ indTrack ][ TrackStatus_source[ indTrack ].indReading ]) > 100 ) ) {
	   isCurveBreak = true;
	}
	...
	tracks_RadiusY[ indTrack ] = (float)TrackRadiusY_source[ indTrack ][ indFrameReading ]
      ...
      // ****************************************************************************
      // TO BE DELETED TEMPORARY CODE FOR THE TEASER
#ifdef PG_ALKEMI_TEASER
      TrackStatus_source[ indTrack ].readSpeedScale = 1.0;

      // printf("mouse pos %dx%d %d\n",CurrentMousePos_x,CurrentMousePos_y,indTrack);
      tracks_Color_r[ indTrack ] = local_color_r
		  + color_spectrum[0] * color_spectrum_coef;
      tracks_Color_g[ indTrack ] = local_color_g
		  + color_spectrum[1] * color_spectrum_coef;
      tracks_Color_b[ indTrack ] = local_color_b
		  + color_spectrum[2] * color_spectrum_coef;
      tracks_Color_a[ indTrack ] = local_color_a
		  + color_spectrum[3];
       // printf("Track %d color a %.2f %.2f %.2f %.2f\n" , indTrack, local_color_r
	 //+ color_spectrum[1] , 
	 //local_color_g 
	 //+ color_spectrum[1] , 
	 //local_color_b 
	 ...
	 //local_color_a
	 //+ color_spectrum[3]  ); 
      tracks_BrushID[ indTrack ] = brushID;
#ifdef WACOM_TABLET
      tracks_RadiusX[ indTrack ] 
	= brushRadius+ (indTrack == 0 ? brush_radius_vol * brush_radius_vol_coef:0.0f)
	  + (withTabletPressureRadius ? tabletPressureRadius * brush_radius_pressure_coef: 0.0f)
	  + (withTabletAngleRadius ? fabs( sin(tabletAzimutRadius) )  * tabletInclinationRadius * brush_radius_angleHor_coef: 0.0f);
      tracks_RadiusY[ indTrack ]
	= brushRadius+ (indTrack == 0 ? brush_radius_vol * brush_radius_vol_coef:0.0f)
	  + (withTabletPressureRadius ? tabletPressureRadius * brush_radius_pressure_coef: 0.0f)
	  + (withTabletAngleRadius ? fabs( cos(tabletAzimutRadius) ) * tabletInclinationRadius * brush_radius_angleVer_coef: 0.0f);
#else
     tracks_RadiusX[ indTrack ] = brushRadius
       + (indTrack == 0 ? brush_radius_vol * brush_radius_vol_coef: 0.0f);
      tracks_RadiusY[ indTrack ] = brushRadius
       + (indTrack == 0 ? brush_radius_vol * brush_radius_vol_coef: 0.0f);
#endif
#endif
      // TO BE DELETED TEMPORARY CODE FOR THE TEASER
  ...
  if( !is_trackreplay ) {
	...
	= brushRadius+ (currentTrack == 0 ? brush_radius_vol * brush_radius_vol_coef:0.0f)
     ...
     tracks_RadiusX[ currentTrack ] = brushRadius
       + (currentTrack == 0 ? brush_radius_vol * brush_radius_vol_coef: 0.0f);
...
void readsvg( int *fileDepth , int indTrack , char *fileName ,
  ...
  while( true ) {
	...
	while ( found==std::string::npos && !fin.eof() ) {      
	  ...
	  LoadTrackFromPath( (char *)val.c_str() , indTrack , translation , isSource , &indSpline );
