...
void pg_loadAllShaders( void ) {
  ...
  pg_loadshader( (char *)("ProjectsBefore18/" + project_name + "/shaders/LYM_Drawing_"+project_name+"-VP.glsl").c_str() , pg_vs_Drawing);
  glCompileShader (pg_vs_Drawing);
  pg_printLog(pg_vs_Drawing);

  unsigned int pg_vs_Composition = glCreateShader (GL_VERTEX_SHADER);
  pg_loadshader( (char *)("ProjectsBefore18/" + project_name + "/shaders/LYM_Composition_"+project_name+"-VP.glsl").c_str() , pg_vs_Composition);
  glCompileShader (pg_vs_Composition);
  pg_printLog(pg_vs_Composition);

  unsigned int pg_vs_Final = glCreateShader (GL_VERTEX_SHADER);
  pg_loadshader( (char *)("ProjectsBefore18/" + project_name + "/shaders/LYM_Final_"+project_name+"-VP.glsl").c_str() , pg_vs_Final);
  glCompileShader (pg_vs_Final);
  pg_printLog(pg_vs_Final);

#ifdef PG_TRACK_DISPLAY
  unsigned int pg_vs_TrackDisplay = glCreateShader (GL_VERTEX_SHADER);
  pg_loadshader( (char *)("ProjectsBefore18/" + project_name + "/shaders/LYM_TrackDisplay_"+project_name+"-VP.glsl").c_str() , pg_vs_TrackDisplay);
  ...
  pg_loadshader( (char *)("ProjectsBefore18/" + project_name + "/shaders/LYM_TrackDisplay_"+project_name+"-GS.glsl").c_str() , pg_gs_TrackDisplay);
  ...
  pg_loadshader( (char *)("ProjectsBefore18/" + project_name + "/shaders/LYM_Drawing_"+project_name+"-FS_full.glsl").c_str() , pg_fs_Drawing);
  glCompileShader (pg_fs_Drawing);
  pg_printLog(pg_fs_Drawing);

  unsigned int pg_fs_Composition = glCreateShader (GL_FRAGMENT_SHADER);
  pg_loadshader( (char *)("ProjectsBefore18/" + project_name + "/shaders/LYM_Composition_"+project_name+"-FS_full.glsl").c_str() , pg_fs_Composition);
  glCompileShader (pg_fs_Composition);
  pg_printLog(pg_fs_Composition);

  unsigned int pg_fs_Final = glCreateShader (GL_FRAGMENT_SHADER);
  pg_loadshader( (char *)("ProjectsBefore18/" + project_name + "/shaders/LYM_Final_"+project_name+"-FS_full.glsl").c_str() , pg_fs_Final);
  glCompileShader (pg_fs_Final);
  pg_printLog(pg_fs_Final);

#ifdef PG_TRACK_DISPLAY
  unsigned int pg_fs_TrackDisplay = glCreateShader (GL_FRAGMENT_SHADER);
  pg_loadshader( (char *)("ProjectsBefore18/" + project_name + "/shaders/LYM_TrackDisplay_"+project_name+"-FS_full.glsl").c_str() , pg_fs_TrackDisplay);
