...
int main(int argcMain, char **argvMain) {
  ...
  pg_ReadAllDisplayMessages("./moire/messages/","message_list.txt");
...
//   printf("-----------------------------------------------------------\n");
  ...
  for( int indContext = 0 ; indContext < PG_MAX_NUMBER_OF_OPENGL_CONTEXTS ; indContext++ ) {
	...
	BOOL error = wglShareLists(MainOpenGLRenderingContext, SharedOpenGLRenderingContext[indContext]);
    if (error == FALSE ) {
		printOglError( 234 );
...
#ifdef PG_VIDEO_ACTIVE
  // release camera
  cvReleaseCapture( &pg_input_camera );
#endif
  ...
  if( windowDisplayed ) {
	...
	if (CurrentScene > 0 && CurrentScene < NbTotScenes - 1) {
		if (DisplayText1Front) {
			// increase text1 alpha up to 1 and text2 alpha down to 0 (cross fading text2 -> text1)
			if (text_swap_duration > 0.0f) {
				DisplayText1Alpha = std::min(1.0f, (float)(CurrentClockTime - DisplayTextSwapInitialTime) / text_swap_duration);
			...
			if (text_swap_duration > 0.0f) {
				DisplayText2Alpha = std::min(1.0f, (float)(CurrentClockTime - DisplayTextSwapInitialTime) / text_swap_duration);
    ...
    if( auto_beat && CurrentClockTime > lastAutoBeatTime + beat_duration ) {
      ...
      BeatNo++;
      lastAutoBeatTime += beat_duration;
      if( lastAutoBeatTime > CurrentClockTime + beat_duration
