...
std::string ImageDir = "./moire/images";
std::string MaskDir = "./moire/masks";
...
bool pg_initTextures( void ) {
  ...
  pg_loadTexture3D( (char *)(project_name+"/textures/Pen_"+project_name+"_3D").c_str() , 
		 ".png" , 1 , 4 , true ,
  ...
  strcpy(pDataTrack->fname, (char *)(project_name + "/textures/trackBrushes.png").c_str());
  ...
  pg_loadTexture3D( (char *)(project_name+"/textures/ParticleAcceleration_"+project_name+"_3D").c_str() , 
		 ".png" , 7 , 4 , true ,
...
void* pg_readImageData(void * lpParam) {
	...
	if (!img.data) {
		sprintf(ErrorStr, "Could not open or find the 2D image %s!", pDataArray->fname); // ReportError(ErrorStr); throw 425;
	...
	if (img.cols != pDataArray->w || img.rows != pDataArray->h) {
		sprintf(ErrorStr, "Unexpected image size %s %d/%d %d/%d!",
			pDataArray->fname, pDataArray->img->cols, pDataArray->w, pDataArray->img->rows,
			pDataArray->h);/// ReportError(ErrorStr); throw 425;
		pDataArray->loadedInRAM = false;
		img.release();
		*(pDataArray->fname) = 0;
		return 0;
...
bool ImageBufferDataStruct::pg_loadImage( bool toBeInverted , int width , 
								int height , bool threaded , bool verbose ) {
	bool valret = true;
...
bool pg_load_and_swap_image(std::string * fileName, double duration) {
	...
	if ((indAvailableSwapImage = available_swap_image_buffer(IndInitialSwapImage)) >= 0) {
		...
		if (!image_swap_buffer_data[indAvailableSwapImage].pg_loadImage(
			true, leftWindowWidth, window_height, true, false)) {
			return false;
		}
...
bool  pg_ReadInitalImageTextures( int ind_dir ) {
	...
	//sprintf(string_number, "_%03d", (int)file_index);
	...
	for(int ind = 0 ; ind < PG_IMAGE_NB_TEXTURES; ind++ ) {
	  image_buffer_weights[ind] = (float)(ind + 1) / PG_IMAGE_NB_TEXTURES;
	}
			...
			if (!image_mask_buffer_data[indBuffer].IDallocated) {
				...
				if ((fileName = nextFile(&MaskDir, &indDir, &indFile))) {
					...
					valret &= image_mask_buffer_data[indBuffer].pg_loadImage(
						true, leftWindowWidth, window_height, true, true);
					delete fileName;
					fileName = NULL;
				}
			}
