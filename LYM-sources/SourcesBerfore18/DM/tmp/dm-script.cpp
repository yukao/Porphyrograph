...
const char *BaseActionIDString[EmptyBaseActionID + 1] = { "set_node_attribute_value", "set_node_scalar_value" , "set_node_table_value" , "set_node_matrix_value" , "set_environment_attribute_value", "set_user_attribute_value" , "set_schedule_attribute_value", "set_material_attribute_value" , "set_video_attribute_value" , "set_internal_state" , "set_viewpoint" , "set_current_viewpoint" , "execute_message" , "send_message" , "forward_message" , "send_message_udp" , "forward_message_udp" , "write_log_file" , "write_console" ,  "write_frame" ,  "screenshot" ,  "write_message" ,  "exit" , "execute_system_command" , "flush_delayed_actions" , "EmptyBaseActionID" };
...
bool dm_BaseAction::ParseBaseAction( int *fileDepth , int *curChar , 
  ...
  case screenshot:
    ...
    if( XMLtag->hasAttribute( "type" ) ) {
      ...
      if( strcmp( targetStringValueAux , "png" ) != 0 
...
void dm_BaseAction::execute( dm_SceneNode **targetNodes , int nbTargetNodes ,
  ...
  case set_schedule_attribute_value:
	...
	if( (targetNode = targetNodes[ind]) ) {
	  ...
	  else if( targetNode->type == AInterpolator ) {
	    dm_NodeInterpolator * interpolator 
	      = ((dm_NodeInterpolator *)(targetNode->sceneObject));
	    if( targetIntValue < 0 || 
		targetIntValue >= interpolator->nbSchedules ) {
	      delete tempTag;
	      tempTag = NULL;
	      sprintf( ErrorStr , "Error: incorrect schedule rank [%d/%d]!" , targetIntValue + 1 , interpolator->nbSchedules ); ReportError( ErrorStr ) ; break;
	    }
	    targetSchedule = interpolator->schedules[targetIntValue];
	  }
  ...
  case set_internal_state:
	...
	if( (targetNode = targetNodes[ind]) ) {
	  ...
	  if( targetNode->type == AInterpolator  ) {
	    dm_NodeInterpolator *interp = ((dm_NodeInterpolator *)(targetNode->sceneObject));
	    if( interp && interp->curNode ) {	    
	      StrCpy( &(interp->curNode->internalState) , tempString );
	      targetNode->forceUpdate( true );
	    }
	  }
	}
  ...
  case set_viewpoint:
	...
	if( (targetNode = targetNodes[ind]) 
	  ...
	  if( targetNode->type == AUser ) {
	    ...
	    if( interp 
		&& interp->curNode
		&& interp->curNode->type == AUser ) {	    
	      dm_User * user = ((dm_User *)(interp->curNode->sceneObject));
	      user->manualViewpointControl = true;
	      user->indCurrentViewpoint = targetIntValue % DM_CompositionNode->nbRegisteredViewpoints;
	      // 	      if( user->indCurrentViewpoint == 0 ) {
	      // 		// the case of navigation viewpoint
  ...
  case screenshot:
	...
	sprintf( ErrorStr , "Incorrect screenshot type (%s): expected png or jpg!" , targetStringValueAux ); ReportError( ErrorStr );
      }
    }
  case write_console:
    {
...
void dm_Command::StoresOSCParameters( char *pch ) {
  ...
  while( indOSCParam < commandNbOSCParams && p_c ) {
	...
	= (float) ReadFloat( NULL , &p_c , true ,pch , &indChar , 
      ...
      char valueString[XMLAttrValLength];
      // printf( "ReadStringWithFrontiers2 p_c %c\n" , p_c );
      if( p_c == '"' ) {
	ReadStringWithFrontiers( NULL , &p_c , valueString ,
				 XMLAttrValLength , '"' , '"' , true , 
				 pch , &indChar );
      }
      else {
	ReadStringWithTermination( NULL , &p_c , valueString ,
				   XMLAttrValLength , ' ' , true , 
				   pch , &indChar );
      }
...
void dm_SceneNode::gatherScripts( dm_Command **commands ) {
  ...
  // collecting scripts on the elements of an interpolator
  if( type == AInterpolator && sceneObject ) {
    dm_NodeInterpolator * aInterpolator = (dm_NodeInterpolator *)sceneObject;
    for( int ind = 0 ; ind < aInterpolator->nbComponents ; ind++ ) {
      aInterpolator->nodes[ind]->gatherScripts( commands );
    }
  }
...
void dm_SceneNode::findAllScriptReferences( dm_SceneNode *rootNode, char * targetID ) {
  ...
  // resolving references on scripts of the elements of an interpolator
  // and for its completion command if there is one
  if( type == AInterpolator && sceneObject ) {
    dm_NodeInterpolator * aInterpolator = (dm_NodeInterpolator *)sceneObject;
    for( int ind = 0 ; ind < aInterpolator->nbComponents ; ind++ ) {
      aInterpolator->nodes[ind]->findAllScriptReferences( rootNode, targetID );
    }
    aInterpolator->findAllScriptReferences( rootNode, targetID );
  }
