  ...
  while( true ) {
	...
	*p_c = ReadChar(fileDepth,charstring,ind);
	...
	if( fileDepth ) {
	  ...
	  char  tempID[XMLAttrChLength];
	  memset( tempID , 0 , XMLAttrChLength );
...
void     ReadID( int *fileDepth , int *p_c , char *p_targetstring , 
  ...
  if (_Alphanum(*p_c) || *p_c == '.' ) {
    while ( (_Alphanum(*p_c) || *p_c == '.') && i < p_size - 1 ) {
      p_targetstring[i++] = *p_c;
      *p_c = ReadChar(fileDepth,charstring,ind);
    }
      
    if( i == p_size - 1) {
      while (_Alphanum(*p_c) || *p_c == '.') {
	*p_c = ReadChar(fileDepth,charstring,ind);
      }
    }
    p_targetstring[i] = '\0';
...
int     ReadStringWithTermination( int *fileDepth , int *p_c , 
  ...
  while( *p_c != p_frontierRight && i < p_size - 1  && *p_c != EOF ) {
    if( *p_c == '\\' ) {
      *p_c = ReadChar(fileDepth,charstring,ind);
...
int     ReadStringWithFrontiers( int *fileDepth , int *p_c , 
  ...
  if( i == p_size - 1) {
    sprintf( ErrorStr , "Error: max string size exceeded (max %d chars)!" , p_size ); ReportError( ErrorStr ); 
    while( *p_c != p_frontierRight && *p_c != EOF ) {
      if( *p_c == '\\' ) {
	*p_c = ReadChar(fileDepth,charstring,ind);
      }
...
bool  dm_XMLTag::ParseTagXML( int *fileDepth , int *p_c ,
  ...
  temporaryAttribute = new char[XMLAttrChLength];
  memset( temporaryAttribute , 0 , XMLAttrChLength );
  temporaryValue = new char[XMLAttrValLength];
  memset( temporaryValue , 0 , XMLAttrValLength );
  ...
  if( *p_c == '/' ) {
    ...
    ReadIDWithCarSpec( fileDepth , p_c , temporaryAttribute , 
		       XMLAttrChLength , "?:_-" , true , charstring,ind );
...
//     }
	...
	*arraySubscript = 0;
	...
	shad->getParameters( attrOrVar , 
      ...
      // special case of an interpolator that can have a schedule
      if( valDataType == EmptyDataType 
	  && referenceNode->type == AInterpolator 
	  && referenceNode->sceneObject ) {
	dm_NodeInterpolator *aInterpolator 
	  = (dm_NodeInterpolator *)(referenceNode->sceneObject);
	if( *attrOrVar == 's' && strchr( attrOrVar , ':' ) ) {
	  int ind = atoi( attrOrVar + 1 ) - 1;
	  if( ind < aInterpolator->nbSchedules ) {
	    aInterpolator->schedules[ ind ]
	      ->getParameters( strchr( attrOrVar , ':' ) + 1 , 
			       &valDataType ,
			       &valDouble ,
			       &valInt ,
			       &valBool ,
