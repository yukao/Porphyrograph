...
"""
This script imports a Wavefront XML files to Blender.

Usage:
Run this script from "File->Import" menu and then load the desired XML file.
Note, This loads mesh objects and materials only, nurbs and curves are not supported.

http://wiki.blender.org/index.php/Scripts/Manual/Import/wavefront_xml
"""
...
def xml_image_load(imagepath, DIR, recursive):
    '''
...
def create_materials(filepath, material_libs, unique_materials, unique_material_images, use_image_search):
    '''
    Create all the used materials in this xml,
    ...
    def load_material_image(blender_material, context_material_name, imagepath, type):
        ...
        image = xml_image_load(imagepath, DIR, use_image_search)
        has_data = False
    ...
    # Add an MTL with the same name as the xml if no MTLs are spesified.
            ...
            for line in mtl:  # .readlines():
                ...
                if not line or line.startswith(b'#'):
                    ...
                    if line_lower.startswith(b'ka'):
                        ...
                        """
                        # XXX, this is not following the XML spec, but this was
...
def get_float_func(filepath):
    '''
    find the float function for this xml file
...
def load(operator, context, filepath,
    ...
    load_xml(path) - should give acceptable results.
    This function passes the file and sends the data off
        to be split into objects and then converted into mesh objects
    '''
    print('\nimporting xml %r' % filepath)
...
# NOTES (all line numbers refer to 2.4x import_xml.py, not this file)
...
# replaced BPyImage.comprehensiveImageLoad with a simplified version that only checks additional directory specified, but doesn't search dirs recursively (xml_image_load)
