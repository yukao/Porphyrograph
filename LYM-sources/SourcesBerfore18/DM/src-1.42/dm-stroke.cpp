...
#ifdef CityWalk
	// 2018 MODIFICATION CJ
	scalar_brushRadius = rootNode->getScalarFromName("local_radius");
	if (scalar_brushRadius == NULL) {
		sprintf(ErrorStr, "Root node scalar local_radius not found!"); ReportError(ErrorStr); throw 207;
	}
	scalar_brush_radius_vol = rootNode->getScalarFromName("local_radius_vol_coef");
	if (scalar_brush_radius_vol == NULL) {
		sprintf(ErrorStr, "Root node scalar local_radius_vol_coef found!"); ReportError(ErrorStr); throw 207;
	}
#endif
	// 2018 MODIFICATION CJ
...
void updateMouseEnvironmentVariablesAndTables( double theTime ) {
  ...
  if( currentTrack >= 0
	...
	+ (currentTrack == 0 ? 
	   (float)scalar_brush_radius_vol->GetValue()->valFloat:0.0f);
  ...
  if( DM_EnvironmentNode->tracks_replay_table->GetValue( currentTrack )->valFloat != 1.0F ) {
	...
	SetValue( (float)table_local_color->GetNumericalValue( 3 )
	 ...
	 + (float)table_color_spectrum->GetNumericalValue( 3 )  ); */
      DM_EnvironmentNode->tracks_BrushID_table->
	SetValue( (float)scalar_brushID->GetValue()->valInt , 
		  currentTrack );
      DM_EnvironmentNode->tracks_Radius_table->
