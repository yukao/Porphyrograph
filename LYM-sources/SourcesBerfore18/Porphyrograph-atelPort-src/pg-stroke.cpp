...
void pg_initStrokes( void ) {
  ...
  pg_Path_Time = new double*[PG_NB_PATHS + 1];
  ...
  for( int ind = 0 ; ind < PG_NB_PATHS + 1; ind++ ) {
    ...
    pg_Path_Status[ ind ].initialTimeRecording = 0.0;
    pg_Path_Status[ ind ].initialTimeReading = 0.0;
...
void LoadPathFromXML(char *pathString, int indPath,
	...
	pg_Path_Status[indPath].initialTimeRecording = 0.0;
	pg_Path_Status[indPath].initialTimeReading = 0.0;
...
void updateMouseEnvironmentVariablesAndTables(double theTime) {
		...
		for (int indRecordingPath = 1; indRecordingPath <= PG_NB_PATHS; indRecordingPath++) {
			if (pg_Path_Status[indRecordingPath].isActiveRecording
				...
				pg_Path_Color_b[indRecordingPath][indFrameRec]
					= pen_grey
					+ pulsed_pen_color[2];
				pg_Path_Color_a[indRecordingPath][indFrameRec]
					= pen_color_a;
	...
	for (int indPath = 1; indPath < PG_NB_PATHS + 1; indPath++) {
		...
		if (is_path_replay[indPath] && indPath >= 3) {
#else
		if (is_path_replay[indPath] && indPath >= 1) {
...
#ifdef ATELIERS_PORTATIFS
			// sustain progressively going down
			if (indPath == 4) {
				if (pg_CurrentScene == 1 || pg_CurrentScene == 3 || pg_CurrentScene == 5) {
					paths_y[indPath] += 0;
				}
				else if (pg_CurrentScene == 10) {
					paths_y[indPath] += 140;
				}
				else if (pg_CurrentScene == 12) {
					paths_y[indPath] += 320;
				}
				else if (pg_CurrentScene == 14) {
					paths_y[indPath] += 255;
					paths_x[indPath] += 140;
				}
				else if (pg_CurrentScene == 16) {
					paths_y[indPath] += 500;
					paths_x[indPath] += 140;
				}
			}
			...
			paths_RadiusY[indPath] = (float)pg_Path_RadiusY[indPath][indFrameReading]
				+ pulse_average * pen_radius_replay_pulse;

#ifdef ATELIERS_PORTATIFS
			// non visible strokes for some paths
			if (indPath == 4  /*sustain*/ || indPath == 5  /*doppler*/ || indPath == 6 /*glissando*/) {
				paths_RadiusX[indPath] = 0.f;
				paths_RadiusY[indPath] = 0.f;
			}
#endif
...
#ifdef ATELIERS_PORTATIFS
		// plays the enveloppe in the middle of the screen
		else {
			if ((path_replay_trackNo_1 >= 0 && indPath == 1) || (path_replay_trackNo_2 >= 0 && indPath == 2)) {
				float initialRamp;
				if (pg_FrameNo <= pg_LaunchFrameNo) {
					initialRamp = 0.f;
				}
				else if (pg_FrameNo - pg_LaunchFrameNo <= 100.f) {
					initialRamp = (pg_FrameNo - pg_LaunchFrameNo) / 100.f;
				}
				else {
					initialRamp = 1.f;
				}
				...
				paths_y_prev[indPath] = paths_y[indPath];
				if (indPath == 1) {
					paths_x[indPath] = initialRamp * ((pulse_average <= 1.f ? (pulse_average >= 0.f ? pulse_average : 0.f) : 1.f) - 0.5f) * 140
						+ (leftWindowWidth / 2)
						// middle height of the screen
						+ (randomValue - 0.5f) / 10.f;
					// random value added so that it is drawn even for stable x,y
					paths_y[indPath] = float(window_height / 2 - 1);
				}
				else if (indPath == 2) {
					paths_x[indPath] = initialRamp * ((pulse_attack <= 1.f ? (pulse_attack >= 0.f ? pulse_attack : 0.f) : 1.f) - 0.5f) * 140
						+ (leftWindowWidth / 2)
						// middle height of the screen
						+ (randomValue - 0.5f) / 10.f;
					// random value added so that it is drawn even for stable x,y
				...
				paths_Color_r[indPath] = 1.0f;
				paths_Color_g[indPath] = 1.0f;
				paths_Color_b[indPath] = 1.0f;
				paths_Color_a[indPath] = 1.0f;
				paths_BrushID[indPath] = 0;
				paths_RadiusX[indPath] = 1.0f;
				paths_RadiusY[indPath] = 1.0f;
			}
		}
#endif
	}
	...
	paths_Color_r[0] = pen_grey
		+ pulsed_pen_color[0];
	paths_Color_g[0] = pen_grey
		+ pulsed_pen_color[1];
	paths_Color_b[0] = pen_grey
		+ pulsed_pen_color[2];
	paths_Color_a[0] = min(1.f, pen_color_a + pulsed_pen_color[3]);
...
void load_svg_path(char *fileName, int indPath, int indTrack,
	...
	if (indPath >= 1
		...
		case 1:
			path_replay_trackNo_1 = indTrack;
			path_record_1 = false;
			break;
		case 2:
			path_replay_trackNo_2 = indTrack;
			path_record_2 = false;
			break;
		case 3:
			path_replay_trackNo_3 = indTrack;
			path_record_3 = false;
			break;
#endif
#if PG_NB_PATHS == 7
		case 4:
			path_replay_trackNo_4 = indTrack;
			path_record_4 = false;
			break;
		case 5:
			path_replay_trackNo_5 = indTrack;
			path_record_5 = false;
			break;
		case 6:
			path_replay_trackNo_6 = indTrack;
			path_record_6 = false;
			break;
		case 7:
			path_replay_trackNo_7 = indTrack;
			path_record_7 = false;
			break;
#endif
		}
