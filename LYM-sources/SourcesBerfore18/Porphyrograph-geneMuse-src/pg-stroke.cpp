...
void pg_initStrokes( void ) {
  ...
  for( int ind = 0 ; ind < PG_NB_TRACKS ; ind++ ) {
    TrackColor_r_source[ ind ] 
      = new float[ PG_EnvironmentNode->Nb_max_mouse_recording_frames ];
    ...
    TrackBrushID_source[ ind ] 
      = new int[ PG_EnvironmentNode->Nb_max_mouse_recording_frames ];
    ...
    TrackTime_source[ ind ] 
      = new double[ PG_EnvironmentNode->Nb_max_mouse_recording_frames ];
...
void pg_trackVariablesInit( void ) {
  ...
  // external tablet
  tracks_x[PG_NB_TRACKS] = 0.0f;
  tracks_y[PG_NB_TRACKS] = 0.0f;
  tracks_x_prev[PG_NB_TRACKS] = 0.0f;
  tracks_y_prev[PG_NB_TRACKS] = 0.0f;
  tracks_RadiusX[PG_NB_TRACKS] = 3.0f;
  tracks_RadiusY[PG_NB_TRACKS] = 3.0f;
}
...
void NormalizeTargetTrack( int indTrack ) {
  if( TrackStatus_target[ 0 ].isNormalized ) {
    return;
  }
  if( TrackStatus_source[ 0 ].nbRecordedFrames <= 4 
      || TrackStatus_target[ 0 ].nbRecordedFrames <= 4 ) {
    TrackStatus_source[ 0 ].isActiveInterpolation = false;
    return;
  }

  for( int indFrameRec = 0 ; 
       indFrameRec < TrackStatus_target[ 0 ].nbRecordedFrames ;
       indFrameRec++ ) {
    // copies all the target frame values
    TrackColor_r_temp_target[ 0 ][ indFrameRec ] = TrackColor_r_target[ 0 ][ indFrameRec ]; 
    TrackColor_g_temp_target[ 0 ][ indFrameRec ] = TrackColor_g_target[ 0 ][ indFrameRec ]; 
    TrackColor_b_temp_target[ 0 ][ indFrameRec ] = TrackColor_b_target[ 0 ][ indFrameRec ]; 
    TrackColor_a_temp_target[ 0 ][ indFrameRec ] = TrackColor_a_target[ 0 ][ indFrameRec ]; 
    TrackBrushID_temp_target[ 0 ][ indFrameRec ] = TrackBrushID_target[ 0 ][ indFrameRec ]; 
    TrackRadiusX_temp_target[ 0 ][ indFrameRec ] = TrackRadiusX_target[ 0 ][ indFrameRec ]; 
    TrackRadiusY_temp_target[ 0 ][ indFrameRec ] = TrackRadiusY_target[ 0 ][ indFrameRec ]; 
    TrackPos_x_temp_target[ 0 ][ indFrameRec ] = TrackPos_x_target[ 0 ][ indFrameRec ]; 
    TrackPos_y_temp_target[ 0 ][ indFrameRec ] = TrackPos_y_target[ 0 ][ indFrameRec ]; 
    TrackTime_temp_target[ 0 ][ indFrameRec ] = TrackTime_target[ 0 ][ indFrameRec ];
  }

  double sourceDuration = TrackTime_source[ 0 ][ TrackStatus_source[ 0 ].nbRecordedFrames - 1 ]
    - TrackStatus_source[ 0 ].initialTimeRecording;
  double targetDuration = TrackTime_target[ 0 ][ TrackStatus_target[ 0 ].nbRecordedFrames - 1 ]
    - TrackStatus_target[ 0 ].initialTimeRecording;
  ...
  TrackTime_target[ 0 ][ 0 ] = TrackTime_source[ 0 ][ 0 ];

  // copies the data of the temporary stored target frame
  TrackColor_r_target[ 0 ][ 0 ] = TrackColor_r_temp_target[ 0 ][ 0 ]; 
  TrackColor_g_target[ 0 ][ 0 ] = TrackColor_g_temp_target[ 0 ][ 0 ]; 
  TrackColor_b_target[ 0 ][ 0 ] = TrackColor_b_temp_target[ 0 ][ 0 ]; 
  TrackColor_a_target[ 0 ][ 0 ] = TrackColor_a_temp_target[ 0 ][ 0 ]; 
  TrackBrushID_target[ 0 ][ 0 ] = TrackBrushID_temp_target[ 0 ][ 0 ]; 
  TrackRadiusX_target[ 0 ][ 0 ] = TrackRadiusX_temp_target[ 0 ][ 0 ]; 
  TrackRadiusY_target[ 0 ][ 0 ] = TrackRadiusY_temp_target[ 0 ][ 0 ]; 
  TrackPos_x_target[ 0 ][ 0 ] = TrackPos_x_temp_target[ 0 ][ 0 ]; 
  TrackPos_y_target[ 0 ][ 0 ] = TrackPos_y_temp_target[ 0 ][ 0 ]; 
  ...
  for(int indSourceFrame = 1 ; 
    ...
    while( nextTargetElapsedTime < sourceElapsedTime 
	   && indTargetFrame + 2 < TrackStatus_target[ 0 ].nbRecordedFrames ) {
      indTargetFrame++;
      previousTargetElapsedTime = nextTargetElapsedTime;
      nextTargetElapsedTime = timeRatio * (TrackTime_temp_target[ 0 ][ indTargetFrame + 1 ] 
				       - TrackStatus_target[ 0 ].initialTimeRecording);
    }
    ...
    if( !((previousTargetElapsedTime <= sourceElapsedTime)
      ...
      printf("Error: the current source time is not contained between two target times\nsource time %f target times %f %f index source %d index target %d size source %d size target %d\n" ,
	     ...
	     indSourceFrame , indTargetFrame ,TrackStatus_source[ 0 ].nbRecordedFrames , TrackStatus_target[ 0 ].nbRecordedFrames);
    ...
    TrackColor_r_target[ 0 ][ indSourceFrame ]
      = Lerp( TrackColor_r_temp_target[ 0 ][ indTargetFrame ] , 
	      TrackColor_r_temp_target[ 0 ][ indTargetFrame + 1 ] , interpolationStepf );
    TrackColor_g_target[ 0 ][ indSourceFrame ]
      = Lerp( TrackColor_g_temp_target[ 0 ][ indTargetFrame ] , 
	      TrackColor_g_temp_target[ 0 ][ indTargetFrame + 1 ] , interpolationStepf );
    TrackColor_b_target[ 0 ][ indSourceFrame ]
      = Lerp( TrackColor_b_temp_target[ 0 ][ indTargetFrame ] , 
	      TrackColor_b_temp_target[ 0 ][ indTargetFrame + 1 ] , interpolationStepf );
    TrackColor_a_target[ 0 ][ indSourceFrame ]
      = Lerp( TrackColor_a_temp_target[ 0 ][ indTargetFrame ] , 
	      TrackColor_a_temp_target[ 0 ][ indTargetFrame + 1 ] , interpolationStepf );
    TrackRadiusX_target[ 0 ][ indSourceFrame ]
      = Lerp( TrackRadiusX_temp_target[ 0 ][ indTargetFrame ] , 
	      TrackRadiusX_temp_target[ 0 ][ indTargetFrame + 1 ] , interpolationStepf );
    TrackRadiusY_target[ 0 ][ indSourceFrame ]
      = Lerp( TrackRadiusY_temp_target[ 0 ][ indTargetFrame ] , 
	      TrackRadiusY_temp_target[ 0 ][ indTargetFrame + 1 ] , interpolationStepf );

    // upper value of brush ID
    TrackBrushID_target[ 0 ][ indSourceFrame ]
      = TrackBrushID_temp_target[ 0 ][ indTargetFrame + 1 ];

    // spline intperpolation for position
    vec2 p0,p1,p2,p3,p_interp;
    // point before previous one
    if( indTargetFrame > 0 ) {
      p0.x = (float)TrackPos_x_temp_target[ 0 ][ indTargetFrame - 1 ];
      p0.y = (float)TrackPos_y_temp_target[ 0 ][ indTargetFrame - 1 ];
    }
    else {
      p0.x = (float)TrackPos_x_temp_target[ 0 ][ 0 ];
      p0.y = (float)TrackPos_y_temp_target[ 0 ][ 0 ];
    }
    // previous point (before current source frame time)
    p1.x = (float)TrackPos_x_temp_target[ 0 ][ indTargetFrame ];
    p1.y = (float)TrackPos_y_temp_target[ 0 ][ indTargetFrame ]; 
    // next point (after current source frame time)
    if( indTargetFrame + 1 < TrackStatus_target[ 0 ].nbRecordedFrames ) {
      p2.x = (float)TrackPos_x_temp_target[ 0 ][ indTargetFrame + 1 ];
      p2.y = (float)TrackPos_y_temp_target[ 0 ][ indTargetFrame + 1 ];
    }
    else {
      p2.x = (float)TrackPos_x_temp_target[ 0 ][ TrackStatus_target[ 0 ].nbRecordedFrames - 1 ];
      p2.y = (float)TrackPos_y_temp_target[ 0 ][ TrackStatus_target[ 0 ].nbRecordedFrames - 1 ];
    }
    // point after next
    if( indTargetFrame + 2 < TrackStatus_target[ 0 ].nbRecordedFrames ) {
      p3.x = (float)TrackPos_x_temp_target[ 0 ][ indTargetFrame + 2 ];
      p3.y = (float)TrackPos_y_temp_target[ 0 ][ indTargetFrame + 2 ];
    }
    else {
      p3.x = (float)TrackPos_x_temp_target[ 0 ][ TrackStatus_target[ 0 ].nbRecordedFrames - 1 ];
      p3.y = (float)TrackPos_y_temp_target[ 0 ][ TrackStatus_target[ 0 ].nbRecordedFrames - 1 ];
    }
    // spline interpolator to smooth the curve
    CatmullRom_Evaluate( &p_interp, &p0, &p1, &p2, &p3, interpolationStepf);
    TrackPos_x_target[ 0 ][ indSourceFrame ] = (float)round(p_interp.x);
    TrackPos_y_target[ 0 ][ indSourceFrame ] = (float)round(p_interp.y);
    ...
    if( indSourceFrame > 1 ) {
      float tan_x = Tension * ( TrackPos_x_target[ 0 ][ indSourceFrame ] - TrackPos_x_target[ 0 ][ indSourceFrame - 2 ]);
      float tan_y = Tension * ( TrackPos_y_target[ 0 ][ indSourceFrame ] - TrackPos_y_target[ 0 ][ indSourceFrame - 2 ]); 
      TrackPos_xL_target[ 0 ][ indSourceFrame - 1 ] 
	= TrackPos_x_target[ 0 ][ indSourceFrame - 1 ] - tan_x;
      TrackPos_yL_target[ 0 ][ indSourceFrame - 1 ]  
	= TrackPos_y_target[ 0 ][ indSourceFrame - 1 ] - tan_y;
      TrackPos_xR_target[ 0 ][ indSourceFrame - 1 ] 
	= TrackPos_x_target[ 0 ][ indSourceFrame - 1 ] + tan_x;
      TrackPos_yR_target[ 0 ][ indSourceFrame - 1 ]  
	= TrackPos_y_target[ 0 ][ indSourceFrame - 1 ] + tan_y;
    ...
    // copies the current time in the current frame
    TrackTime_target[ 0 ][ indSourceFrame ]
      = TrackTime_source[ 0 ][ indSourceFrame ];
  }

  // copies last frame of both
  int indSourceFrame = TrackStatus_source[ 0 ].nbRecordedFrames - 1;
  indTargetFrame = TrackStatus_target[ 0 ].nbRecordedFrames - 1;
  ...
  // copies the data of the temporary stored target frame
  TrackColor_r_target[ 0 ][ indSourceFrame ] = TrackColor_r_temp_target[ 0 ][ indTargetFrame ]; 
  TrackColor_g_target[ 0 ][ indSourceFrame ] = TrackColor_g_temp_target[ 0 ][ indTargetFrame ]; 
  TrackColor_b_target[ 0 ][ indSourceFrame ] = TrackColor_b_temp_target[ 0 ][ indTargetFrame ]; 
  TrackColor_a_target[ 0 ][ indSourceFrame ] = TrackColor_a_temp_target[ 0 ][ indTargetFrame ]; 
  TrackBrushID_target[ 0 ][ indSourceFrame ] = TrackBrushID_temp_target[ 0 ][ indTargetFrame ]; 
  TrackRadiusX_target[ 0 ][ indSourceFrame ] = TrackRadiusX_temp_target[ 0 ][ indTargetFrame ]; 
  TrackRadiusY_target[ 0 ][ indSourceFrame ] = TrackRadiusY_temp_target[ 0 ][ indTargetFrame ]; 
  TrackPos_x_target[ 0 ][ indSourceFrame ] = TrackPos_x_temp_target[ 0 ][ indTargetFrame ]; 
  TrackPos_y_target[ 0 ][ indSourceFrame ] = TrackPos_y_temp_target[ 0 ][ indTargetFrame ]; 

  // marks the track as normalized
  TrackStatus_target[ 0 ].isNormalized = true;
}
...
void LoadTrackFromPath( char *pathString , int indTrack , 
  ...
  if( isSource ) {
    TrackStatus_source[ 0 ].isFirstFrame = false;
    TrackStatus_source[ 0 ].isActiveInterpolation = false;
    TrackStatus_source[ 0 ].isActiveRecording = false;
    TrackStatus_source[ 0 ].isNormalized = false;
    TrackStatus_source[ 0 ].nbRecordedFrames = 0;
    TrackStatus_source[ 0 ].indReading = 0;
    TrackStatus_source[ 0 ].coefInterpolation = 0;
    TrackStatus_source[ 0 ].initialTimeRecording = 0.0;
    TrackStatus_source[ 0 ].initialTimeReading = 0.0;
    TrackStatus_source[ 0 ].isStepwiseInterpolation = false;
    TrackStatus_source[ 0 ].interpolationStep = 0.00001F;
    TrackStatus_source[ 0 ].readSpeedScale = 1.0F;
  }
  else {
    TrackStatus_target[ 0 ].isFirstFrame = false;
    TrackStatus_target[ 0 ].isActiveInterpolation = false;
    TrackStatus_target[ 0 ].isActiveRecording = false;
    TrackStatus_target[ 0 ].isNormalized = false;
    TrackStatus_target[ 0 ].nbRecordedFrames = 0;
    TrackStatus_target[ 0 ].indReading = 0;
    TrackStatus_target[ 0 ].coefInterpolation = 0;
    TrackStatus_target[ 0 ].initialTimeRecording = 0.0;
    TrackStatus_target[ 0 ].initialTimeReading = 0.0;
    TrackStatus_target[ 0 ].isStepwiseInterpolation = false;
    TrackStatus_target[ 0 ].interpolationStep = 0.00001F;
    TrackStatus_target[ 0 ].readSpeedScale = 1.0F;
  }
  ...
  while( indChar <= (int)strlen( pathString ) && curChar != EOF ) {
	...
	while( _Num(curChar) || curChar == '-' || curChar == '+' ) { 
	  ...
	  SpaceCommentsInclude( NULL , &curChar , pathString , &indChar );
	  nbMovePoints++;
	  if( indSpline < PG_EnvironmentNode->Nb_max_mouse_recording_frames ) {
	    if( isSource ) {
	      TrackColor_r_source[ 0 ][ indSpline ] = 1.0;
	      TrackColor_g_source[ 0 ][ indSpline ] = 1.0;
	      TrackColor_b_source[ 0 ][ indSpline ] = 1.0;
	      TrackColor_a_source[ 0 ][ indSpline ] = 1.0;
	      TrackBrushID_source[ 0 ][ indSpline ] = 0;
	      TrackRadiusX_source[ 0 ][ indSpline ] = 0.0;
	      TrackRadiusY_source[ 0 ][ indSpline ] = 0.0;
	      TrackPos_x_source[ 0 ][ indSpline ] = currentPoint[0] + translation[0];
	      TrackPos_y_source[ 0 ][ indSpline ] = currentPoint[1] + translation[1];
	      TrackTime_source[ 0 ][ indSpline ] = indSpline * interFrameDuration;
	      // printf("src move to %d %f %f\n" , indSpline ,
	      // 	     TrackPos_x_source[ 0 ][ indSpline ] , 
	      // 	     TrackPos_y_source[ 0 ][ indSpline ]);
	    }
	    else {
	      TrackColor_r_target[ 0 ][ indSpline ] = 1.0;
	      TrackColor_g_target[ 0 ][ indSpline ] = 1.0;
	      TrackColor_b_target[ 0 ][ indSpline ] = 1.0;
	      TrackColor_a_target[ 0 ][ indSpline ] = 1.0;
	      TrackBrushID_target[ 0 ][ indSpline ] = 0;
	      TrackRadiusX_target[ 0 ][ indSpline ] = 0.0;
	      TrackRadiusY_target[ 0 ][ indSpline ] = 0.0;
	      TrackPos_x_target[ 0 ][ indSpline ] = currentPoint[0] + translation[0];
	      TrackPos_y_target[ 0 ][ indSpline ] = currentPoint[1] + translation[1];
	      TrackTime_target[ 0 ][ indSpline ] = indSpline * interFrameDuration;
	...
	while( _Num(curChar) || curChar == '-' || curChar == '+' ) { 
	  ...
	  if( (nbCurvePoints) % 3 == 0  && indSpline < PG_EnvironmentNode->Nb_max_mouse_recording_frames ) {	    
	    if( isSource ) {
	      TrackColor_r_source[ 0 ][ indSpline ] = 1.0;
	      TrackColor_g_source[ 0 ][ indSpline ] = 1.0;
	      TrackColor_b_source[ 0 ][ indSpline ] = 1.0;
	      TrackColor_a_source[ 0 ][ indSpline ] = 1.0;
	      TrackBrushID_source[ 0 ][ indSpline ] = 0;
	      TrackRadiusX_source[ 0 ][ indSpline ] = 3.0;
	      TrackRadiusY_source[ 0 ][ indSpline ] = 3.0;
	      TrackPos_x_source[ 0 ][ indSpline ] = currentPoint[0] + translation[0];
	      TrackPos_y_source[ 0 ][ indSpline ] = currentPoint[1] + translation[1];
	      TrackTime_source[ 0 ][ indSpline ] = indSpline * interFrameDuration;
	      // if( indSpline < 5 ) {
	  ...
	  else if( (nbCurvePoints) % 3 == 1  && indSpline > 0 && indSpline < PG_EnvironmentNode->Nb_max_mouse_recording_frames ) {
	    if( isSource ) {
	  ...
	  else if( (nbCurvePoints) % 3 == 2  && indSpline < PG_EnvironmentNode->Nb_max_mouse_recording_frames ) {
	    if( isSource ) {
...
void updateMouseEnvironmentVariablesAndTables( double theTime ) {
  ...
  if( currentTrack >= 0
      && ( (mouse_x_prev != CurrentMousePos_x)
    ...
    ///////////////////////////////////////////////////////////////////////
    if( TrackStatus_source[ currentTrack ].isActiveRecording 
	&& TrackStatus_source[ currentTrack ].nbRecordedFrames 
	< PG_EnvironmentNode->Nb_max_mouse_recording_frames ) {
      int indFrameRec = TrackStatus_source[ currentTrack ].nbRecordedFrames;
      // printf( "rec track %d frame %d\n",currentTrack,indFrameRec );
	
      // records the initial time and the current time to play in synch
      if( indFrameRec == 0 ) {
	TrackStatus_source[ currentTrack ].initialTimeRecording = theTime;
      }
      TrackTime_source[ currentTrack ][ indFrameRec ] = theTime;
	
      TrackColor_r_source[ currentTrack ][ indFrameRec ] 
	= local_color_r
	+ color_spectrum[0] * color_spectrum_coef;
      TrackColor_g_source[ currentTrack ][ indFrameRec ]
	= local_color_g
	+ color_spectrum[1] * color_spectrum_coef;
      TrackColor_b_source[ currentTrack ][ indFrameRec ]
	= local_color_b
	+ color_spectrum[2] * color_spectrum_coef;
      TrackColor_a_source[ currentTrack ][ indFrameRec ]
	= local_color_a;
      ...
      TrackBrushID_source[ currentTrack ][ indFrameRec ] 
	= brushID;

#ifdef WACOM_TABLET
      TrackRadiusX_source[ currentTrack ][ indFrameRec ] 
	= brushRadius + brush_radius_vol * brush_radius_vol_coef
      ...
      TrackRadiusY_source[ currentTrack ][ indFrameRec ] 
	= brushRadius + brush_radius_vol * brush_radius_vol_coef
	  + tabletPressureRadius * brush_radius_pressure_coef
	...
	= brushRadius + brush_radius_vol * brush_radius_vol_coef;
      ...
      TrackPos_x_source[ currentTrack ][ indFrameRec ] = (float)CurrentMousePos_x;
      TrackPos_y_source[ currentTrack ][ indFrameRec ] = (float)CurrentMousePos_y;
	...
	float tan_x = Tension * ( TrackPos_x_source[ currentTrack ][ indFrameRec ] - TrackPos_x_source[ currentTrack ][ indFrameRec - 2 ]);
	float tan_y = Tension * ( TrackPos_y_source[ currentTrack ][ indFrameRec ] - TrackPos_y_source[ currentTrack ][ indFrameRec - 2 ]); 
	// move second step: null left tangent of current point
	if( !wasAMove ) {
	  TrackPos_xL_source[ currentTrack ][ indFrameRec - 1 ] 
	    = TrackPos_x_source[ currentTrack ][ indFrameRec - 1 ] - tan_x;
	  TrackPos_yL_source[ currentTrack ][ indFrameRec - 1 ]  
	    = TrackPos_y_source[ currentTrack ][ indFrameRec - 1 ] - tan_y;
	}
	else {
	  TrackPos_xL_source[ currentTrack ][ indFrameRec - 1 ] = 0.0;
	  TrackPos_yL_source[ currentTrack ][ indFrameRec - 1 ] = 0.0;
	...
	if( !isAMove ) {
	  if( !wasAMove ) {
	    TrackPos_xR_source[ currentTrack ][ indFrameRec - 1 ] 
	      = TrackPos_x_source[ currentTrack ][ indFrameRec - 1 ] + tan_x;
	    TrackPos_yR_source[ currentTrack ][ indFrameRec - 1 ]  
	      = TrackPos_y_source[ currentTrack ][ indFrameRec - 1 ] + tan_y;
	  }
	  else {
	    // first point in a new curve
	    TrackPos_xR_source[ currentTrack ][ indFrameRec - 1 ] 
	      = TrackPos_x_source[ currentTrack ][ indFrameRec - 1 ];
	    TrackPos_yR_source[ currentTrack ][ indFrameRec - 1 ]  
	      = TrackPos_y_source[ currentTrack ][ indFrameRec - 1 ];
	  }	      
	}
	else {
	  // assigns null values to right tangent extremity so that it is saved
	  // as a move
	  TrackPos_xR_source[ currentTrack ][ indFrameRec - 1 ] = 0.0;
	  TrackPos_yR_source[ currentTrack ][ indFrameRec - 1 ] = 0.0;
	  // a null left tangent extremity will be givent at next frame

	  // null left tangent at closing point before the move
	  TrackPos_xL_source[ currentTrack ][ indFrameRec - 1 ] = TrackPos_x_source[ currentTrack ][ indFrameRec - 1 ];
	  TrackPos_yL_source[ currentTrack ][ indFrameRec - 1 ] = TrackPos_y_source[ currentTrack ][ indFrameRec - 1 ];
	}

	// right tangent is half way between first point and left tangent extremity of second point
	if( indFrameRec == 2 ) {
	  TrackPos_xR_source[ currentTrack ][ 0 ] = TrackPos_x_source[ currentTrack ][ 0 ] 
	    + 0.5F * ( TrackPos_xL_source[ currentTrack ][ 1 ] - TrackPos_x_source[ currentTrack ][ 0 ]); 
	  TrackPos_yR_source[ currentTrack ][ 0 ] = TrackPos_y_source[ currentTrack ][ 0 ] 
	    + 0.5F * ( TrackPos_yL_source[ currentTrack ][ 1 ] - TrackPos_y_source[ currentTrack ][ 0 ]); 
	}
      }	
      // printf( "Track src rec %d Ind %d tan %d %d / prev pos %d %d\n" , currentTrack + 1 , indFrameRec - 1 , (int)TrackPos_xL_source[ currentTrack ][ indFrameRec - 1 ] , (int)TrackPos_yL_source[ currentTrack ][ indFrameRec - 1 ] , (int)TrackPos_xR_source[ currentTrack ][ indFrameRec - 1 ] , (int)TrackPos_yR_source[ currentTrack ][ indFrameRec - 1 ] );
...
#ifdef WACOM_TABLET
      TrackRadiusX_target[ currentTrack ][ indFrameRec ] 
	= brushRadius+ brush_radius_vol * brush_radius_vol_coef
	  + tabletPressureRadius * brush_radius_pressure_coef
	  + fabs( sin(tabletAzimutRadius) )  * tabletInclinationRadius * brush_radius_angleHor_coef;
      TrackRadiusY_target[ currentTrack ][ indFrameRec ] 
	= brushRadius+ brush_radius_vol * brush_radius_vol_coef
	  + tabletPressureRadius * brush_radius_pressure_coef
	  + fabs( cos(tabletAzimutRadius) ) * tabletInclinationRadius * brush_radius_angleVer_coef;
#else
  ...
  for( int indTrack = 0 ; 
	...
	tracks_Color_r[ indTrack ] = (float)Lerp((float)TrackColor_r_source[ indTrack ][ indFrameReading ] , 
		  ...
		  + color_spectrum[0] * color_spectrum_replay_coef;
	tracks_Color_g[ indTrack ] =  (float)Lerp((float)TrackColor_g_source[ indTrack ][ indFrameReading ] , 
		  ...
		  + color_spectrum[1] * color_spectrum_replay_coef;
	tracks_Color_b[ indTrack ] =  (float)Lerp((float)TrackColor_b_source[ indTrack ][ indFrameReading ] , 
		  ...
		  + color_spectrum[2] * color_spectrum_replay_coef;
	...
	tracks_RadiusX[ indTrack ] = (float)Lerp((float)TrackRadiusX_source[ indTrack ][ indFrameReading ] , 
       ...
       + (indTrack == 0 ? brush_radius_vol * brush_radius_vol_replay_coef : 0.0f);
  ...
  if( !is_trackreplay ) {
	...
	&& isDrawingOn ) {
      // printf("mouse pos %dx%d %d\n",CurrentMousePos_x,CurrentMousePos_y,currentTrack);
      tracks_Color_r[ currentTrack ] = local_color_r
		  + color_spectrum[0] * color_spectrum_coef;
      tracks_Color_g[ currentTrack ] = local_color_g
		  + color_spectrum[1] * color_spectrum_coef;
      tracks_Color_b[ currentTrack ] = local_color_b
		  + color_spectrum[2] * color_spectrum_coef;
      tracks_Color_a[ currentTrack ] = local_color_a
		  + color_spectrum[3];
  //      printf("track %d color %.2f %.2f %.2f a:%.2f coef:%.2f spectrum:  %.2f %.2f %.2f\n" , currentTrack, local_color_r
	 //+ color_spectrum[0] * color_spectrum_coef , 
	 //local_color_g 
	 //+ color_spectrum[1] * color_spectrum_coef , 
	 //local_color_b 
	 //+ color_spectrum[2] * color_spectrum_coef , 
	 //local_color_a
	 //+ color_spectrum[3] , color_spectrum_coef , color_spectrum[0] , color_spectrum[1] , color_spectrum[2] ); 
      tracks_BrushID[ currentTrack ] = brushID;
#ifdef WACOM_TABLET
      tracks_RadiusX[ currentTrack ] 
	= brushRadius+ brush_radius_vol * brush_radius_vol_coef
	  + tabletPressureRadius * brush_radius_pressure_coef
	  + fabs( sin(tabletAzimutRadius) )  * tabletInclinationRadius * brush_radius_angleHor_coef;
      tracks_RadiusY[ currentTrack ]
	= brushRadius+ brush_radius_vol * brush_radius_vol_coef
	  + tabletPressureRadius * brush_radius_pressure_coef
	  + fabs( cos(tabletAzimutRadius) ) * tabletInclinationRadius * brush_radius_angleVer_coef;
#else
     tracks_RadiusX[ currentTrack ] = brushRadius
       + brush_radius_vol * brush_radius_vol_coef;
      tracks_RadiusY[ currentTrack ] = brushRadius
       + brush_radius_vol * brush_radius_vol_coef;
#endif
      // printf("Track %d brush ID radius %d %.2f\n" , currentTrack, brushID, brushRadius );
    }
  }
...
void pg_writeTrackTextures( int indTrack , bool is_target ) {
  ...
  if( !is_target && TrackStatus_source[ indTrack ].nbRecordedFrames > 0 ) {
    ...
    glTexImage1D(GL_TEXTURE_1D,     // Type of texture
		 ...
		 PG_EnvironmentNode->Nb_max_mouse_recording_frames ,          // Image width
