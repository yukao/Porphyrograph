...
void pg_IPClient::Init( void ) {
  ...
  // remote server opening
	...
	int err = getaddrinfo(Remote_server_IP, Remote_server_port_string, &hints, &result);
	if (err != 0) {
		sprintf(ErrorStr, "Error: unknown remote host IP '%s (%d)'!", Remote_server_IP, err); ReportError(ErrorStr);
	}
	else {
	  char                    hname[1024];
	  err = getnameinfo(
	  ...
	  if (err != 0) {
		  ...
		  // deprecated Remote_server_host = gethostbyname(Remote_server_IP);
		  ...
		  printf("Network client: sending data to '%s' (%s) on port %d\n",
			  Remote_server_IP, hname, Remote_server_port);
...
void pg_IPClient::SetId( char *newId ) {
  StrCpy( &id , newId );
}

void pg_IPClient::setParameters( pg_XMLTag *XMLtag , bool atRunTime ,
...
bool pg_IPClient::getParameters( char *attribute , 
				    DataType * dataType ,
				    double * valDouble ,
				    int * valInt ,
				    bool * valBool ,
...
void pg_IPClient::print( FILE *file , int isLong , int depth ) {
  indent( file , depth );
...
void pg_IPClient::sendIPmessages( void ) {
  ...
  while( current_depth_output_stack >= 0 ) {
	...
	if( PG_EnvironmentNode->IP_Servers[ ind ]->last_received_IP_message_number 
	...
	if( send_ID ) {
	  ...
	  strncat( Output_Message_String , output_message_stack[ 0 ] , 
		   PG_EnvironmentNode->Max_network_message_length 
	  ...
	  if( *send_prefix ) {
	    ...
	    strncpy( Output_Message_String , output_message_stack[ 0 ] , 
		     PG_EnvironmentNode->Max_network_message_length - 1 );
	  }
	}
      }
	...
	if( *(output_message_stack[ 0 ]) != '/' ) {
	  ...
	  if( send_ID ) {
	    ...
	    strncat( Output_Message_OSC + 20 , output_message_stack[ 0 ] , 
		     2 * PG_EnvironmentNode->Max_network_message_length 
	    ...
	    strncat( Output_Message_OSC + 20 , output_message_stack[ 0 ] , 
	      ...
	      strncpy( Output_Message_OSC + 20 , output_message_stack[ 0 ] , 
		       2 * PG_EnvironmentNode->Max_network_message_length - 21 );
	    }
	  }
	...
	// message that begins by an OSC address /aaaa/aaa 
	  ...
	  if( hasPattern ) {
	    ...
	    if( (indLocalCommandLine + 4 + nbArg + indArgList )
		 < (2 * PG_EnvironmentNode->Max_network_message_length - 1 ) ) {
...
void pg_IPServer::Init( void ) {
  ...
  for( int ind = 0 ; ind < MAX_OSC_ARGUMENTS ; ind++ ) {
    OSC_arguments[ind] = new char[ XMLAttrValLength ];
    *(OSC_arguments[ind]) = 0;
  }
  OSCTag = new char[ XMLAttrValLength ];
}
...
void pg_IPServer::SetId( char *newId ) {
  StrCpy( &id , newId );
}

void pg_IPServer::setParameters( pg_XMLTag *XMLtag , bool atRunTime ,
				  FloatBinOp oper , IntBinOp operI ) {
...
bool pg_IPServer::getParameters( char *attribute , 
				    DataType * dataType ,
				    double * valDouble ,
				    int * valInt ,
				    bool * valBool ,
...
void pg_IPServer::print( FILE *file , int isLong , int depth ) {
  indent( file , depth );
...
int pg_IPServer::receiveOneMessage( char *message ) {
  ...
  n = recv(SocketToLocalServer, message, 
	   PG_EnvironmentNode->Max_network_message_length, 0);

  if( carbon_copyIPClient ) {
    int rc;
	rc = send(carbon_copyIPClient->SocketToRemoteServer, message,
		n, 0);
...
void pg_IPServer::storeIP_input_messages_and_removes_duplicates( 
		      char *message ) {
  if( SocketToLocalServer < 0 ) {
    return;
  }
