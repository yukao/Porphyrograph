...
void pg_IPClient::InitClient( void ) {
  ...
  if (err != 0) {
    ...
    // emission counter, used to stop emission beyond 10 failures
	nbEmissionFailures = 0;
...
void pg_IPClient::IP_OutputStackInitialization( void ) {
  ...
  // printf("Initialized output stack size %d\n", depth_output_stack);
}

void pg_IPClient::sendIPmessages( void ) {
  if( SocketToRemoteServer < 0 || nbEmissionFailures > 10 ) {
    return;
  }

  while( current_depth_output_stack >= 0 && nbEmissionFailures <= 10 ) {
	...
	// message that begins by an OSC address /aaaa/aaa
	  ...
	  if( hasPattern ) {
		...
		// printf( "ReadStringWithFrontiers p_c %c\n" , p_c );
	        char temporaryValue[ PG_MESSAGE_LENGTH ];
		*temporaryValue = 0;
		if( messageOut[ inChSource ] == '"' ) {
		  int i = 0;
		  p_c = messageOut[ inChSource++ ];
		  while( p_c != '"' && i < PG_MESSAGE_LENGTH - 1  && p_c != 0 ) {
		    temporaryValue[i++] = p_c;
		    p_c = messageOut[ inChSource++ ];
		  }
		  if( i == PG_MESSAGE_LENGTH - 1) {
		    sprintf( ErrorStr , "Error: max string size exceeded ReadStringWithFrontiers (max %d chars)!" , PG_MESSAGE_LENGTH ); ReportError( ErrorStr ); throw(100);
		  }
		  temporaryValue[i] = '\0';
		  p_c = messageOut[ inChSource++ ];
		  while( _SpaceChar(p_c) ) {
	...
	if(rc != localCommandLineLength) {
      ...
      if(nbEmissionFailures == 1) {
        ...
        ReportError( (char *)"Error: failure in UDP connection!" );
      }
	}
...
void pg_IPServer::InitServer( void ) {
  ...
  for( int ind = 0 ; ind < MAX_OSC_ARGUMENTS ; ind++ ) {
    OSC_arguments[ind] = new char[ PG_MESSAGE_LENGTH ];
    *(OSC_arguments[ind]) = 0;
  }
  OSCTag = new char[ PG_MESSAGE_LENGTH ];
}
...
void pg_IPServer::processReceivedOSC( char *localCommandLine ,
  ...
  if( indChar < n && message[indChar] == ',' ) {
    ...
    int nbParam = 0;
    char paramTypesId[ PG_MESSAGE_LENGTH ];
    char argument[ PG_MESSAGE_LENGTH ];
    while( indChar < n && message[indChar] != 0
	   && nbParam < PG_MESSAGE_LENGTH ) {
