...
import vv_scenario_transformation
import vv_scenario_layer_crossfading
	...
	for ind_scene in range(nb_scenes) :
		...
		for indImageInput in range(val_init["imageInputIndex"], val_fin["imageInputIndex"], pas):
			...
			for indNewClipActivation in range(len(active_performing_clips), nb_transformed_performing_clips) :
				...
				fill_color = interpolate ("hexcolor", fill_color_min, fill_color_max, "l", random())
				stroke_color = interpolate ("hexcolor", stroke_color_min, stroke_color_max, "l", random())
				randVal = random()
				...
				active_performing_clips[keyNewActiveClip] \
					= [newActivatedClipID, newActivatedClipSize, current_crossfaded_frame_out, \
						fill_color, stroke_color, stroke_width, scaling_factor, VV_SVG_PAGE_WIDTH / 2, 0]
			...
			for key in sorted(active_performing_clips):
				...
				transformation_scenario_file_name = tmp_dir_name + "LYM_Arachnid_scenario_transform-single_clip.csv"
				...
				if not TRACE_ONLY:
					...
					if vv_scenario_transform.main(["-i", \
							"/mnt/d/sync.com/Sync/LYM-videos-sources/LYM_Annika_Arachnid_2020/SVG_movies/tmp/LYM_Arachnid_scenario_transform-single_clip.csv"]) == 0:
						return 0
			...
			if not TRACE_ONLY:
				...
				if vv_scenario_layer_crossfading.main(["-i", \
						"/mnt/d/sync.com/Sync/LYM-videos-sources/LYM_Annika_Arachnid_2020/SVG_movies/tmp/LYM_Arachnid_scenario_layer_crossfade.csv"]) == 0:
					return 0
			...
			end_transformation_scenario_file_name = tmp_dir_name + "LYM_Arachnid_scenario_end_transform.csv"
			...
			if not TRACE_ONLY:
				...
				if vv_scenario_transformation.main(["-i", \
						"/mnt/d/sync.com/Sync/LYM-videos-sources/LYM_Annika_Arachnid_2020/SVG_movies/tmp/LYM_Arachnid_scenario_end_transform.csv"]) == 0:
					return 0
