...
##################################################################
TouchOSC_IP = "192.168.137.159" # the IPaddress on the phone
...
class Source:
	...
	def __init__(self, ind):
		...
		self.radius = 0.5
		self.ratio = 0.5
		self.sourcespeed = 0.01
		self.trajectoryspeed = 0.01
	...
	def step(self, indStep):
		...
		self.source_angle = self.source_angle + self.sourcespeed
		# updates trajectory_angle
		self.trajectory_angle = self.trajectory_angle + self.trajectoryspeed
		# updates position
...
def record_params() :
	...
	with open('F:/Ableton/TouchOSC_2020/spat_default_setups/setup_record'+date_time+'.csv', 'wt', newline='') as csvfile:
		...
		values = []
		for source in list(Sources.values()) :
			values = values + [str(source.sourcespeed), str(source.trajectoryspeed), \
			str(source.radius), str(source.ratio), str(source.source_angle), \
			str(source.trajectory_angle)]
		logwriter.writerow(values)
	csvfile.close()
	...
	with open( 'F:/Ableton/TouchOSC_2020/spat_default_setups/setup.csv', 'rt', newline='') as csvfile:
		...
		for row in logreader:
			if(lineNo == indParam) :
				indSource = 0
				for source in list(Sources.values()) :
					source.sourcespeed = to_num(row[indSource * 6 + 0])
					source.trajectoryspeed = to_num(row[indSource * 6 + 1])
					source.radius = to_num(row[indSource * 6 + 2])
					source.ratio = to_num(row[indSource * 6 + 3])
					source.source_angle = to_num(row[indSource * 6 + 4])
					source.trajectory_angle = to_num(row[indSource * 6 + 5])
					indSource += 1
				break
			lineNo += 1
	csvfile.close()
...
def UDP_receive_radius_handler(address, arg):
	...
	msg = oscbuildparse.OSCMessage(address, None, [source.radius])
...
def UDP_receive_ratio_handler(address, arg):
	...
	msg = oscbuildparse.OSCMessage(address, None, [source.ratio])
	osc_send(msg, "TouchOSC_client")
...
def UDP_receive_sourcespeed_handler(address, arg):
	...
	split_add = address.split('/')
	indSrc = int(split_add[2])
	Sources[indSrc - 1].sourcespeed = float(arg) * SpeedFactor
	msg = oscbuildparse.OSCMessage(address, None, [source.sourcespeed])
	osc_send(msg, "TouchOSC_client")
...
def UDP_receive_trajectoryspeed_handler(address, arg):
	...
	split_add = address.split('/')
	indSrc = int(split_add[2])
	Sources[indSrc - 1].trajectoryspeed = float(arg) * SpeedFactor
	msg = oscbuildparse.OSCMessage(address, None, [source.trajectoryspeed])
	osc_send(msg, "TouchOSC_client")
...
def UDP_receive_onTouchOSC_handler(address, arg):
	...
	msg = oscbuildparse.OSCMessage(address, None, [source.onTouchOSC])
	osc_send(msg, "TouchOSC_client")
...
def OSC_emitter_resend_all():
	...
	for indSrc, source in Sources.items() :
		...
		osc_send(msg, "TouchOSC_client")
		msg = oscbuildparse.OSCMessage("/source/"+str(source.indSource)+"/sourcespeed", None, [source.sourcespeed / SpeedFactor])
		osc_send(msg, "TouchOSC_client")
		msg = oscbuildparse.OSCMessage("/source/"+str(source.indSource)+"/trajectoryspeed", None, [source.trajectoryspeed / SpeedFactor])
		osc_send(msg, "TouchOSC_client")
