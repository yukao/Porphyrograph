...
##################################################################
TouchOSC_IP = "192.168.43.25" # the IPaddress on the phone
...
class Source:
	...
	def step(self, indStep):
		...
		# print("message:", "/source/"+str(self.indSource)+"/xyz", None, [self.y, self.x, 0])
...
def reload_params(configurationFile, indParam) :
	global GlobalSourceSpeed
	global GlobalTrajectorySpeed
	global GlobalStartInterpolation
	global GlobalEndInterpolation

	print("log", configurationFile)
	with open( configurationFile, 'rt', newline='') as csvfile:
...
def UDP_receive_reload_handler(address, arg):
	...
	if(to_num(arg) >= 1) :
		reload_params(ConfigurationFileName, GlobalLastReload)
...
def UDP_globalsourcespeed(address, arg):
	...
	osc_send(msg, "TouchOSC_client")
	print("GlobalSourceSpeed" , GlobalSourceSpeed / SourceSpeedFactor)
...
def main(main_args) :
	...
	try:
		opts, args = getopt.getopt(main_args,"i:s:",["inputfile=","nbsources="])
	...
	for opt, arg in opts:
		...
		elif opt in ("-s", "--nbsources"):
			NumberOfSources = int(arg)
		else:
