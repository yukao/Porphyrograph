...
touchOSC_IP = "192.168.0.101" # for touchOSC
...
_nb_channels_in_control = 14
(_red, _green, _blue, _color, _red, _green, _blue, _grey, _dimmer, _strobe, _zoom, _pan, _tilt, _hue) = range(_nb_channels_in_control)
def command_rank(command_string) :
    if(command_string == "palette_color"):
        return _color
...
class LightGroup:
    ...
    # init method or constructor 
    def __init__(self):
        self.frozen_channels = [False] * _nb_channels_in_control 
        self.lights = []

    def freeze(self, channel):
        if(channel < 0 or channel >= _nb_channels_in_control) :
            print("cannot freeze channel {:d}", channel)
    ...
    def is_frozen(self, channel):
        if(channel != _color): 
            ...
            return self.frozen_channels[_red]
...
def compute_pulsed_palette_color(palette_color, grey = 0) :
    ...
    for indColorBandPass in range(3):
        ...
        rgb_color_percentage[indColorBandPass] = interpolation_color - math.floor(interpolation_color) # decimal part of interpolation_color
    ...
    for indColorBandPass in range(3):
        for indChannel in range(3):
            bandpass_3color_palette[indColorBandPass][indChannel] =  \
                (1. - rgb_color_percentage[indColorBandPass]) * scenario_palette_colors[indLowPalette[indColorBandPass]][indColorBandPass * 3 + indChannel] \
                + (rgb_color_percentage[indColorBandPass]) * scenario_palette_colors[indUpperPalette[indColorBandPass]][indColorBandPass * 3 + indChannel]
...
def DMX_setLightGroup_value(group_no, command, msg_arglist, is_animation) :
    ...
    for light in scenario_light_groups[group_no - 1].lights :
        if(command == _color):
            ...
            if not is_animation :
                scenario_light_groups[group_no - 1].freeze(_red)
        elif(command == _red):
            ...
            if not is_animation :
                scenario_light_groups[group_no - 1].freeze(command)
        elif(command == _dimmer):
            light.dmx_light_set_dimmer(msg_arglist[0])
            if not is_animation :
                scenario_light_groups[group_no - 1].freeze(command)
        elif(command == _strobe):
...
def DMX_initial_value_storage(curve_y_value, address) :
    ...
    is_color = address.endswith("rgb_color")
...
def ANIMATION_DISPLAY_update_values_and_redraw() :
    ...
    for indScene, an_interval in enumerate(sorted_curve_scenes_intervals):
        if indScene == active_scene:
            if alpha_union_intervals < 1 :
                pygame.draw.rect( DISPLAYSURF, pygame.Color(60, 30, 30, 128), 
                                    pygame.Rect(horiz_translate + an_interval[0] * horiz_scale, 0, (an_interval[1] - an_interval[0]) * horiz_scale, 1080), 
                                    width=0)
            else :
                pygame.draw.rect( DISPLAYSURF, pygame.Color(60, 30, 60, 128), 
                                    pygame.Rect(horiz_translate + an_interval[0] * horiz_scale, 0, (an_interval[1] - an_interval[0]) * horiz_scale, 1080), 
                                    width=0)
        else:
            pygame.draw.rect( DISPLAYSURF, pygame.Color(60, 60, 30, 128), 
                                pygame.Rect(horiz_translate + an_interval[0] * horiz_scale, 0, (an_interval[1] - an_interval[0]) * horiz_scale, 1080), 
                                width=0)
        if indScene == active_scene:
        ...
        DISPLAYSURF.blit(text_img, [horiz_translate + an_interval[0] * horiz_scale + 50, 1080 * 0.98])

    # drawing curve ranges
    for curve_range in unique_curve_ranges:
        pygame.draw.rect( DISPLAYSURF, pygame.Color(30, 30, 60, 128), 
                            pygame.Rect(0, curve_range[0] * vertic_scale, 1920, (curve_range[1] - curve_range[0]) * vertic_scale), 
                            width=0)
    ...
    for indScene, (a_curve_scene_indices, an_interval, scene_label) in enumerate(zip(sorted_curve_scenes_indices, sorted_curve_scenes_intervals, scene_labels)):
        ...
        if indScene == active_scene:
            if(y_max * vertic_scale > 1080/2) :
                vertic_translate = - ((y_max * vertic_scale - 1080 * 0.75) // 30 * 30)
            else :
                vertic_translate = 0
        ...
        for curveIndInScene in a_curve_scene_indices:
            ...
            if indScene == active_scene:
                ...
                x = 1920 * 0.05
                y = 1080 * 0.9 - nb_displayed_messages * 24
                astrripped_label = curve_labels[currentCurveIndexInScene][1:]
                [a_light_string, group_no_string, a_command] = astrripped_label.split("/")
                text = "{:<3}: {:<25} {:<10} {:<3}".format(group_no_string, group_IDs[to_num(group_no_string)], a_command, string_value)
    ...
    if active_scene == -1:
        ...
        text_img = font_large.render(text, True, (255,255,255))
        DISPLAYSURF.blit(text_img, [1920 * 0.05, 1080 * 0.5])
        text='{:d} scenes'.format(len(sorted_curve_scenes_intervals))
        text_img = font_large.render(text, True, (255,255,255))
        DISPLAYSURF.blit(text_img, [1920 * 0.05, 1080 * 0.53])
        for indScene, scene_verbatim in enumerate(scene_verbatims):
            text_img = font_medium.render(scene_verbatim, True, (255,255,255))
            last_line = 1080 * (0.56 + 0.02 * indScene)
            DISPLAYSURF.blit(text_img, [1920 * 0.05, last_line])

        line_no = 4
        text_img = font_medium.render("space bar: start/stop, jump to next cue", True, (200,200,255))
        DISPLAYSURF.blit(text_img, [1920 * 0.05, last_line + 1080 * 0.02 * line_no])
        line_no += 1
        text_img = font_medium.render("right/left arrow: jump to next/preceding cue", True, (200,200,255))
        DISPLAYSURF.blit(text_img, [1920 * 0.05, last_line + 1080 * 0.02 * line_no])
        line_no += 1
        text_img = font_medium.render("up/down arrow: speed up/down", True, (200,200,255))
        DISPLAYSURF.blit(text_img, [1920 * 0.05, last_line + 1080 * 0.02 * line_no])
        line_no += 1

    # time cursor
    pygame.draw.line( DISPLAYSURF,pygame.Color('green'), [horiz_translate + cursor_location * horiz_scale, 0], [horiz_translate + cursor_location * horiz_scale, 1080]) #, width=5)  # False is no closing
...
def DISPLAY_current_x_y_values_string(x_interval_percent, curve_y_value, address, is_animation, is_first_value) :
    ...
    if(is_color) : 
        ...
        output_value_list = [curve_y_value]
        string_value = "{:.3f}".format(curve_y_value)
    ...
    return (is_new_value, is_color, output_value_list, string_value, curve_y_value)
...
def OSC_receive_light_control_handler(address, args):
    ...
    # direct light group values for test also
        ...
        if group_no >= 1:
            ...
            address = "/light/{:d}/palette_color".format(group_no)
            if address in previous_address_value.keys() :
                (is_new_value, is_color, output_value_list, string_value, address_value) = DISPLAY_current_x_y_values_string(0, previous_address_value[address][0], address, False, True)
            else :
                (is_new_value, is_color, output_value_list, string_value, address_value) = DISPLAY_current_x_y_values_string(0, 0, address, False, True)
            if(is_new_value) :
                previous_address_value[address] = address_value
                DMX_value_shipping(address, output_value_list, is_color)

            address = "/light/{:d}/red".format(group_no)
            if address in previous_address_value.keys() :
                (is_new_value, is_color, output_value_list, string_value, address_value) = DISPLAY_current_x_y_values_string(0, previous_address_value[address][0], address, False, True)
            else :
                (is_new_value, is_color, output_value_list, string_value, address_value) = DISPLAY_current_x_y_values_string(0, 0, address, False, True)
            ...
            address = "/light/{:d}/green".format(group_no)
            if address in previous_address_value.keys() :
                (is_new_value, is_color, output_value_list, string_value, address_value) = DISPLAY_current_x_y_values_string(0, previous_address_value[address][0], address, False, True)
            else :
                (is_new_value, is_color, output_value_list, string_value, address_value) = DISPLAY_current_x_y_values_string(0, 0, address, False, True)
            ...
            address = "/light/{:d}/blue".format(group_no)
            if address in previous_address_value.keys() :
                (is_new_value, is_color, output_value_list, string_value, address_value) = DISPLAY_current_x_y_values_string(0, previous_address_value[address][0], address, False, True)
            else :
                (is_new_value, is_color, output_value_list, string_value, address_value) = DISPLAY_current_x_y_values_string(0, 0, address, False, True)
            ...
            address = "/light/{:d}/grey".format(group_no)
            if address in previous_address_value.keys() :
                (is_new_value, is_color, output_value_list, string_value, address_value) = DISPLAY_current_x_y_values_string(0, previous_address_value[address][0], address, False, True)
            else :
                (is_new_value, is_color, output_value_list, string_value, address_value) = DISPLAY_current_x_y_values_string(0, 0, address, False, True)
            ...
            address = "/light/{:d}/dimmer".format(group_no)
            if address in previous_address_value.keys() :
                (is_new_value, is_color, output_value_list, string_value, address_value) = DISPLAY_current_x_y_values_string(0, previous_address_value[address][0], address, False, True)
            else :
                (is_new_value, is_color, output_value_list, string_value, address_value) = DISPLAY_current_x_y_values_string(0, 0, address, False, True)
            ...
            address = "/light/{:d}/strobe".format(group_no)
            if address in previous_address_value.keys() :
                (is_new_value, is_color, output_value_list, string_value, address_value) = DISPLAY_current_x_y_values_string(0, previous_address_value[address][0], address, False, True)
            else :
                (is_new_value, is_color, output_value_list, string_value, address_value) = DISPLAY_current_x_y_values_string(0, 0, address, False, True)
            ...
            address = "/light/{:d}/zoom".format(group_no)
            if address in previous_address_value.keys() :
                (is_new_value, is_color, output_value_list, string_value, address_value) = DISPLAY_current_x_y_values_string(0, previous_address_value[address][0], address, False, True)
            else :
                (is_new_value, is_color, output_value_list, string_value, address_value) = DISPLAY_current_x_y_values_string(0, 0, address, False, True)
            ...
            address = "/light/{:d}/pan".format(group_no)
            if address in previous_address_value.keys() :
                (is_new_value, is_color, output_value_list, string_value, address_value) = DISPLAY_current_x_y_values_string(0, previous_address_value[address][0], address, False, True)
            else :
                (is_new_value, is_color, output_value_list, string_value, address_value) = DISPLAY_current_x_y_values_string(0, 0, address, False, True)
            ...
            address = "/light/{:d}/tilt".format(group_no)
            if address in previous_address_value.keys() :
                (is_new_value, is_color, output_value_list, string_value, address_value) = DISPLAY_current_x_y_values_string(0, previous_address_value[address][0], address, False, True)
            else :
                (is_new_value, is_color, output_value_list, string_value, address_value) = DISPLAY_current_x_y_values_string(0, 0, address, False, True)
            ...
            address = "/light/{:d}/hue".format(group_no)
            if address in previous_address_value.keys() :
                (is_new_value, is_color, output_value_list, string_value, address_value) = DISPLAY_current_x_y_values_string(0, previous_address_value[address][0], address, False, True)
            else :
                (is_new_value, is_color, output_value_list, string_value, address_value) = DISPLAY_current_x_y_values_string(0, 0, address, False, True)
    ...
    # direct light group values for test also
        ...
        # print("DMX_value_shipping", address, args, False) 
            ...
            for substring in point_string_list :
                ...
                if(substring.find(',') != -1):
                    ...
                    if is_first_point or absolute :
                        ...
                        cur_point = (cur_point[0]+new_point[0], cur_point[1]+new_point[1])
                ...
                # single number for horizontal or vertical line
                    ...
                    if(horiz):
                        if is_first_point or absolute :
                            cur_point = (float(substring), cur_point[1])
                        else :
                            cur_point = (cur_point[0]+float(substring), cur_point[1])
                        point_list.append(cur_point)
                        is_first_point = False
                    elif(vertic):
                        if is_first_point or absolute :
                            cur_point = (cur_point[0], float(substring))
                        else :
                            cur_point = (cur_point[0], cur_point[1]+float(substring))
                        point_list.append(cur_point)
                        is_first_point = False
...
for (a_curve_scene_indices, an_interval, scene_label) in zip(sorted_curve_scenes_indices, sorted_curve_scenes_intervals, scene_labels):
    scene_verbatim = "scene {:<15s} [{:6.2f}, {:6.2f}]    {:>2d} curves: ".format(scene_label, an_interval[0], an_interval[1], len(a_curve_scene_indices))
    for currentCurveIndexInScene in a_curve_scene_indices:
        address = curve_labels[currentCurveIndexInScene]
        if(curve_inverval_is_reversed[currentCurveIndexInScene]) :
            scene_verbatim = scene_verbatim + "{:2d} {:<25s} ".format(currentCurveIndexInScene, address+"*")
        else:
            scene_verbatim = scene_verbatim + "{:2d} {:<25s} ".format(currentCurveIndexInScene, address+" ")
    # print(scene_verbatim)
    scene_verbatims.append(scene_verbatim)
...
while True:  # loop to wait till window close
    ...
    if(cursor_location * horiz_scale > 1920/2) :
        horiz_translate = - ((cursor_location * horiz_scale - 1920/2) // 30 * 30)
    else :
        horiz_translate = 0
