...
void main() {
 ...
 // cursor type (+1 for stylus and -1 for rubber)
  ...
  vec3 coordsAfterHomography = (fs_homographyForTexture * vec3(decalCoordsPOT.x, 1. - decalCoordsPOT.y, 1));
  ...
  if(photoWeight * uniform_Update_fs_4fv_photo01Wghts_randomValues.x > 0) {
    ...
    if(coordsImageScaled.x > 0.001 && coordsImageScaled.y > 0.001 && coordsImageScaled.x < 0.937  && coordsImageScaled.y < 0.527) { // 1920/2048 and 1080/2048
  ...
  if(photoWeight * uniform_Update_fs_4fv_photo01Wghts_randomValues.y > 0) {
    coordsImage = coordsAfterHomography.xy * uniform_Update_fs_4fv_photo01_wh.zw + uniform_Update_fs_4fv_flashPhotoTrkWght_flashPhotoTrkThres_Photo_offSetsXY.zw;
    // coordsImageScaled = coordsImage;
