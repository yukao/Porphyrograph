/***********************************************************************
File: effe/shaders/LYM_Update_effe-FS.glsl
...
// uniform vec3 uniform_Update_fs_3fv_flashTrack0Coefs;  
uniform vec4 uniform_Update_fs_4fv_flashTrkBGWghts_flashPartBGWght;  

uniform vec4 uniform_Update_fs_4fv_flashTrkCAWghts;  
uniform vec4 uniform_Update_fs_4fv_trackdecay;
uniform vec3 uniform_Update_fs_3fv_CAdecay_frameno_Cursor;

// uniform vec4 uniform_Update_fs_4fv_partAcc_clearAllLayers_clearCA_radiusPart;
...
// uniform vec4 uniform_Update_fs_4fv_movieWH_flashCameCoef_cpTrack;
uniform vec4 uniform_Update_fs_4fv_movieWH_flashCameraTrkWght_cpTrack;

uniform vec4 uniform_Update_fs_4fv_flashBackCoef_repop_Color;
uniform vec4 uniform_Update_fs_4fv_isClearLayer_flashPart;
uniform vec4 uniform_Update_fs_4fv_photo01_wh;
uniform vec2 uniform_Update_fs_2fv_photo01_weights;

/////////////////////////////////////
// INPUT
layout (binding = 0) uniform samplerRect fs_decal;         // 2-cycle ping-pong Update pass BG track step n (FBO attachment 1)
layout (binding = 1) uniform samplerRect fs_lookupTable1;  // 2-cycle ping-pong Update pass speed/position of particles step n (FBO attachment 2)
layout (binding = 2) uniform samplerRect fs_lookupTable2;  // 2-cycle ping-pong Update pass CA step n (FBO attachment 3)
#if PG_NB_TRACKS >= 2
layout (binding = 3) uniform samplerRect fs_lookupTable3;  // 2-cycle ping-pong Update pass track 1 step n (FBO attachment 4)
#endif
#if PG_NB_TRACKS >= 3
layout (binding = 4) uniform samplerRect fs_lookupTable4;  // 2-cycle ping-pong Update pass track 2 step n (FBO attachment 5)
#endif
#if PG_NB_TRACKS >= 4
layout (binding = 5) uniform samplerRect fs_lookupTable5;  // 2-cycle ping-pong Update pass track 3 step n (FBO attachment 6)
#endif
    ...
    if( newDecalCoord.x >= 0 && newDecalCoord.y >= 0 
      ...
      if( graylevel(surrParticle_localColor) >  0 ) {
        ...
        // MOVIE
      	...
      	vec2 acceleration;
      	acceleration = double_accelerations - pixel_acc_center;
      	if( pixel_acc_factor > 0 ) {
      	  // acceleration
      	  surrParticle_speed += pixel_acc_factor * acceleration;
      	}
      	else {
      	  // damping
      	  surrParticle_speed += pixel_acc_factor * surrParticle_speed;
      	}
...
float out_gray_drawing( float current_mouse_x , float current_mouse_y , 
  ...
  if( p_track_drawingStroke == DRAWING_POINT 
    ...
    // in case of point drawing only points are drawn when uniform_Update_fs_4fv_flashTrkBGWghts_flashPartBGWght is not null
    // otherwise the pen is considered out of the screen
    if( p_track_drawingStroke == DRAWING_POINT 
	      && !(currentDrawingTrack > 0 
             && uniform_Update_fs_4fv_flashTrkBGWghts_flashPartBGWght[currentDrawingTrack - 1] > 0) ) {
...
// 0.598 0.362 0.535 0.835 0.512 0.488 0.441 0.724 0.425 0.528 0.48  0.693 0.504 0.488 0.339 0.638 0.276 0.874 0.606 0.228 0.276 0.874 0.606 0.228 0.339 0.709 0.276 0.874 0.606 0.228 0.291 0.283 0.842 0.276 0.874 0.606 0.228 0.165 0.347 0.394
  ...
  // frame number
  frameNo = int(round(uniform_Update_fs_3fv_CAdecay_frameno_Cursor.y));
  ...
  CAdecay = uniform_Update_fs_3fv_CAdecay_frameno_Cursor.x  * (1.0 - CAdecay_pulse * average_pulse);
  ...
  Cursor = uniform_Update_fs_3fv_CAdecay_frameno_Cursor.z;

  // particles
  pixel_acc_center = vec2(pixel_acc_center_0,pixel_acc_center_1);
  radiusParticle = uniform_Update_fs_4fv_clearAllLayers_clearCA_pixelRadius_pulsedShift.z;

  // working variables for screen dimension
  width = uniform_Update_fs_4fv_W_H_drawingStart_drawingStroke.x;
  height = uniform_Update_fs_4fv_W_H_drawingStart_drawingStroke.y;
...
/////////////////////////////////////
  ...
  vec2 decalCoordsPrevStep = decalCoords;
    // + vec2( uniform_Update_fs_3iv_mouseTracks_x_transl[0] ,
	   //  uniform_Update_fs_3iv_mouseTracks_y_transl[0] );
  ...
  if(uniform_Update_fs_2fv_photo01_weights.x > 0) {
    photocolor += uniform_Update_fs_2fv_photo01_weights.x * texture(fs_lookupTable10, 
      vec2(decalCoordsPOT.x , 1.0 - decalCoordsPOT.y) * uniform_Update_fs_4fv_photo01_wh.xy ).rgb;
  }
  if(uniform_Update_fs_2fv_photo01_weights.y > 0) {
    photocolor += uniform_Update_fs_2fv_photo01_weights.y * texture(fs_lookupTable11,  
...
/* EFFE BEFORE SONG
    ...
    if( path_replay_trackNo[indCurTrack] ) {
    ...
    if( track_drawingStroke > 0 ) {
      track_grayLevel[indCurTrack] = out_gray_drawing( uniform_Update_fs_4fv_mouseTracks_x[indCurTrack] , 
  					uniform_Update_fs_4fv_mouseTracks_y[indCurTrack] , 
  					uniform_Update_fs_4fv_mouseTracks_x_prev[indCurTrack] , 
  					uniform_Update_fs_4fv_mouseTracks_y_prev[indCurTrack] ,
  					uniform_Update_fs_4fv_mouseTracks_RadiusX[indCurTrack] ,
  					uniform_Update_fs_4fv_mouseTracks_RadiusY[indCurTrack] ,
  					int(uniform_Update_fs_4fv_mouseTracks_BrushID[indCurTrack]) ,
  					track_drawingStroke ) 
            * uniform_Update_fs_4fv_mouseTracks_a[indCurTrack]; // brush opacity is combined with color opacity
          ...
          out_track_FBO[indCurTrack].rgb 
            = out_track_FBO[indCurTrack].rgb * clamp((1 - track_grayLevel[indCurTrack]),0,1)
            	+ track_grayLevel[indCurTrack]
              	* vec3( uniform_Update_fs_4fv_mouseTracks_r[indCurTrack] , 
              		      uniform_Update_fs_4fv_mouseTracks_g[indCurTrack] , 
              		      uniform_Update_fs_4fv_mouseTracks_b[indCurTrack] );
        }
        // rubber stylus
        else {
...
*/
    ...
    if(currentPhotoTrack == indCurTrack 
      && uniform_Update_fs_2fv_photo01_weights.x + uniform_Update_fs_2fv_photo01_weights.y > 0 ) {
...
/* EFFE BEFORE SONG
    ...
    if( indCurTrack != 0 ) {
      trackColorCopy += uniform_Update_fs_3fv_flashTrack0Coefs[indCurTrack - 1] 
                        * out_track_FBO[indCurTrack].rgb;
    }
    ...
    if( graylevel(out_track_FBO[indCurTrack].rgb) > .2 
      ...
      trackCACopy += uniform_Update_fs_4fv_flashTrkCAWghts[indCurTrack] 
...
*/
    ...
    if( indCurTrack != 0 ) {
      trackColorCopy += uniform_Update_fs_4fv_flashTrkBGWghts_flashPartBGWght[indCurTrack - 1] 
                        * out_track_FBO[indCurTrack].rgb;
    }
  ...
  if( CA_on_off && (frameNo % CAstep == 0) ) {
    ...
    if( repop_CA > 0 ) {
      ...
      if( int(frameNo * randomValue.w ) % int(765 *(1-repop_CA)) == int((randomValue.x * 255.0
        ...
        out4_CA.a = -1.0;
        out4_CA.rgb  = uniform_Update_fs_4fv_flashBackCoef_repop_Color.yzw;
      }
    }
  ...
  // if no flashback uniform_Update_fs_4fv_flashBackCoef_repop_Color.x == 0
  out_track_FBO[0].rgb = clamp( out_track_FBO[0].rgb 
				    + uniform_Update_fs_4fv_flashBackCoef_repop_Color.x * out_CA_FBO.rgb , 
            0 , 1 );
  ...
  if( pixel_mode > 0 ) {
    ...
    //////////////////////////////////////
    //////////////////////////////////////
    if( uniform_Update_fs_4fv_isClearLayer_flashPart.y > 0 ) {
      radiusParticle = 1;
    }
    ...
    // update of the current particle
      ...
      if( pixel_acc_factor > 0 ) {
      	// acceleration
      	out_speed_position_FBO.xy += pixel_acc_factor * acceleration;
      }
      else {
      	// damping
      	out_speed_position_FBO.xy += pixel_acc_factor * out_speed_position_FBO.xy;
      }
  ...
  if( repop_BG > 0 ) {
    ...
    if( int(frameNo * randomValue.w ) % int(765 *(1-repop_BG)) == int((randomValue.x * 255.0
        ...
        out_track_FBO[0].rgb = uniform_Update_fs_4fv_flashBackCoef_repop_Color.yzw;
    }
  }
  ...
  for(int indTrack = 0 ; indTrack < PG_NB_TRACKS ; indTrack++) {
      if( graylevel(out_track_FBO[indTrack].rgb) > 0 ) {
        out_track_FBO[indTrack].rgb 
             = out_track_FBO[indTrack].rgb - vec3(uniform_Update_fs_4fv_trackdecay[indTrack]);
      }
      out_track_FBO[indTrack].rgb 
        = clamp( out_track_FBO[indTrack].rgb , 0.0 , 1.0 );
      // clear layer
      if( currentDrawingTrack == indTrack 
          && uniform_Update_fs_4fv_isClearLayer_flashPart.x > 0 ) {
  ...
  outColor5 = out_track_FBO[0];
#if PG_NB_TRACKS >= 2
  outColor6 = out_track_FBO[1];// + vec4(0,0.1,0,0);
#endif
#if PG_NB_TRACKS >= 3
  outColor7 = out_track_FBO[2];
#endif
