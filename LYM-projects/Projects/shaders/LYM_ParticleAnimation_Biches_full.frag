...
#else
#ifdef PG_VIDEO_ACTIVE
layout (binding = 7) uniform samplerRect uniform_ParticleAnimation_texture_fs_Movie_frame;  // movie textures
#endif
layout (binding = 8) uniform samplerRect uniform_ParticleAnimation_texture_fs_Trk0;  // 2-cycle ping-pong ParticleAnimation pass track 0 step n (FBO attachment 5)
#if PG_NB_TRACKS >= 2
layout (binding = 9) uniform samplerRect uniform_ParticleAnimation_texture_fs_Trk1;  // 2-cycle ping-pong ParticleAnimation pass track 1 step n (FBO attachment 6)
#endif
#if PG_NB_TRACKS >= 3
layout (binding = 10) uniform samplerRect uniform_ParticleAnimation_texture_fs_Trk2;  // 2-cycle ping-pong ParticleAnimation pass track 2 step n (FBO attachment 7)
#endif
#if PG_NB_TRACKS >= 4
layout (binding = 11) uniform samplerRect uniform_ParticleAnimation_texture_fs_Trk3;  // 2-cycle ping-pong Update pass track 3 step n (FBO attachment 8)
#endif
#endif
 ...
 */
  ...
  if(part_initialization >= 0) {
    ...
    float nb_particle_initialization_textures 
      = uniform_ParticleAnimation_fs_4fv_flashCAPartWght_nbPart_clear_nbPartInit.w;
    if(part_initialization < nb_particle_initialization_textures) {
      ...
      */
      { // instant positioning on the target
          ...
          out_color_radius_particle = vec4(target_color_radius.rgb, part_size * (2 * randomValue.z - 0.5));
    ...
    // camera
#ifdef PG_CAMERA_ACTIVE
    else if(part_initialization == nb_particle_initialization_textures ) {
        /* if(partMove_target) { // reaches a target in several steps
          out_target_position_color_radius_particle.xy
...
#endif
    // movie
    else if(part_initialization == nb_particle_initialization_textures + 1 ) {
      /* if(partMove_target) { // reaches a target in several steps
          out_target_position_color_radius_particle.xy
                = texture( uniform_ParticleAnimation_texture_fs_Part_init_pos_speed , decalCoords ).xy;
  ...
  if( out_position_speed_particle.x == -10000) {
    ...
    vec4 randomValue 
    = texture( uniform_ParticleAnimation_texture_fs_Noise , vec3( decalCoordsPOT , 0.25 ) );
    vec4 radius_random 
    = uniform_ParticleAnimation_path_data[0 * PG_MAX_PATH_ANIM_DATA + PG_PATH_ANIM_RAD];
    if( repop_part > 0
  ...
  // damping
  speed2D
    -= dvec2(part_damp * speed2D) * out_color_radius_particle.w;
...
void main() {
  partDecay = uniform_ParticleAnimation_scenario_var_data[0];
  part_initialization = int(uniform_ParticleAnimation_scenario_var_data[1]);
  part_path_follow_0 = (uniform_ParticleAnimation_scenario_var_data[2] > 0 ? true : false);
  part_path_follow_1 = (uniform_ParticleAnimation_scenario_var_data[3] > 0 ? true : false);
  part_path_follow_2 = (uniform_ParticleAnimation_scenario_var_data[4] > 0 ? true : false);
  part_path_follow_3 = (uniform_ParticleAnimation_scenario_var_data[5] > 0 ? true : false);
  part_path_follow_4 = (uniform_ParticleAnimation_scenario_var_data[6] > 0 ? true : false);
  part_path_follow_5 = (uniform_ParticleAnimation_scenario_var_data[7] > 0 ? true : false);
  part_path_follow_6 = (uniform_ParticleAnimation_scenario_var_data[8] > 0 ? true : false);
  part_path_follow_7 = (uniform_ParticleAnimation_scenario_var_data[9] > 0 ? true : false);
  part_path_repulse_0 = (uniform_ParticleAnimation_scenario_var_data[10] > 0 ? true : false);
  part_path_repulse_1 = (uniform_ParticleAnimation_scenario_var_data[11] > 0 ? true : false);
  part_path_repulse_2 = (uniform_ParticleAnimation_scenario_var_data[12] > 0 ? true : false);
  part_path_repulse_3 = (uniform_ParticleAnimation_scenario_var_data[13] > 0 ? true : false);
  part_path_repulse_4 = (uniform_ParticleAnimation_scenario_var_data[14] > 0 ? true : false);
  part_path_repulse_5 = (uniform_ParticleAnimation_scenario_var_data[15] > 0 ? true : false);
  part_path_repulse_6 = (uniform_ParticleAnimation_scenario_var_data[16] > 0 ? true : false);
