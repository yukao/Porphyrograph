...
#include_initializations
 ...
 // cursor type (+1 for stylus and -1 for rubber)
  ...
  float intPart;
#ifdef PG_DOUBLE_SCREEN
  float coordsImageFromScreen = modf(decalCoordsPOT.x * 2, intPart);
#else
  float coordsImageFromScreen = decalCoordsPOT.x;
#endif
  int currentPhotoSource = 0;
  vec2 coordsImageScaled = vec2(0);
  if(photoWeight * uniform_Update_fs_4fv_photo01Wghts_randomValues.x > 0) {
    coordsImage = vec2(coordsImageFromScreen , decalCoordsPOT.y) * uniform_Update_fs_4fv_photo01_wh.xy;
  ...
  if(photoWeight * uniform_Update_fs_4fv_photo01Wghts_randomValues.y > 0) {
    coordsImage = vec2(coordsImageFromScreen , decalCoordsPOT.y) * uniform_Update_fs_4fv_photo01_wh.zw;
  ...
  movieCoord = vec2(coordsImageFromScreen , 1.0-decalCoordsPOT.y )
               * movieWH;
  ...
  cameraCoord = vec2((1 - coordsImageFromScreen), (1 - decalCoordsPOT.y) )
