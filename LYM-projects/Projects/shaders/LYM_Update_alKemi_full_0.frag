...
#define var_freeze
bool	  freeze;
uniform float uniform_Update_scenario_var_data[55];
...
vec2 rotate(vec2 v, float a) {
  ...
  float c = cos(a);
  mat2 m = mat2(c, -s, s, c);
  return m * (v - vec2(0.5, 0.5)) + vec2(0.5, 0.5);
}
vec2 scale(vec2 v, float a) {
  return vec2(a) * (v - vec2(0.5, 0.5)) + vec2(0.5, 0.5);
}
...
void main() {
  ...
  photo_rot = uniform_Update_scenario_var_data[39];
  photo_transl_x = uniform_Update_scenario_var_data[40];
  photo_transl_y = uniform_Update_scenario_var_data[41];
  mask_scale = uniform_Update_scenario_var_data[42];
 ...
 // cursor type (+1 for stylus and -1 for rubber)
  ...
  if(photoWeight * uniform_Update_fs_4fv_photo01Wghts_randomValues.x > 0) {
    coordsImage = scale(rotate(vec2(decalCoordsPOT.x , decalCoordsPOT.y), photo_rot), photo_scale) * uniform_Update_fs_4fv_photo01_wh.xy;
    coordsImage += vec2(photo_transl_x, photo_transl_y)/vec2(width,height);
  ...
  if(photoWeight * uniform_Update_fs_4fv_photo01Wghts_randomValues.y > 0) {
    coordsImage = scale(rotate(vec2(decalCoordsPOT.x , decalCoordsPOT.y), photo_rot), photo_scale) * uniform_Update_fs_4fv_photo01_wh.zw;
    coordsImage += vec2(photo_transl_x, photo_transl_y)/vec2(width,height);
  ...
  for( int indTrack = 1 ; indTrack <= PG_NB_TRACKS ; indTrack++ ) {
    ...
    if(currentPhotoTrack == indCurTrack 
      ...
      // only photo (but not drawing or whatever memory from preceding tracks)
        float photo_grey = graylevel(photocolor);
        float track_grey = graylevel(out_track_FBO[indCurTrack].rgb);
        if(track_grey > 0.3 && photo_grey > 0) {
          out_track_FBO[indCurTrack].rgb = vec3(0);
        }
        else {
          out_track_FBO[indCurTrack].rgb += vec3(photo_grey);
        }
    }
    if( currentPhotoTrack == indCurTrack 
        && flashPhotoTrkWght > 0 
