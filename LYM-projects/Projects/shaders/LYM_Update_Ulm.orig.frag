 ...
 */
  ...
  for( int indTrack = 1 ; indTrack <= PG_NB_TRACKS ; indTrack++ ) {
    ...
    if( currentVideoTrack == indCurTrack && cameraWeight + movieWeight > 0) {
      ...
      if( flashCameraTrkWght > 0 ) { // flash camera
          // video image copy when there is a flash video
        flashToCACumul += flashCameraTrkWght * cameraImage;
      }
      else {
    ...
    if(currentPhotoTrack == indCurTrack 
      ...
      if( flashPhotoTrkWght > 0 ) { // flash photo
        // photo image copy when there is a flash photo
        flashToCACumul += flashPhotoTrkWght * photocolor;
      }
    }
