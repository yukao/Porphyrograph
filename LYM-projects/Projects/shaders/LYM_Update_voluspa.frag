...
vec2 multiTypeGenerativeNoise(vec2 texCoordLocPOT) {
  // FLAT
  if(noiseType == 0 )  {
    return vec2(snoise( texCoordLocPOT , noiseUpdateScale * 100 ),
                            snoise( texCoordLocPOT + vec2(2.0937,9.4872) , noiseUpdateScale * 100 ));
  }
  // SUN RAYS
  else if(noiseType == 1 ) {
      ...
      if( graylevel(surrpixel_localColor) >  0 ) {
        ...
        if(currentScene == 2) {
          ...
          if(acc.x*acc.y != 0) {
            ...
            pixel_acceleration = 1.5 * vec2(snoise( pixelTextureCoordinatesPOT_XY , noiseUpdateScale * 100 ),
