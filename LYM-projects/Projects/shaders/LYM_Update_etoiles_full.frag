    ...
    if( currentCA.a < 0 ) {
      ...
      if(CAType == GOL_1) {
        ...
        if( state < nbStates) {
          ...
          nextColor = clamp(nextColor-vec3(0,CAdecay,CAdecay),0,1);
          if(newState == 0){
            nextColor = vec3(0);
          }
...
// BezierHull the points of the convex hull
...
float out_gray_drawing( float current_Brush_Radius ) {
  ...
  // from the previous mouse position BezierControlX[0] BezierControlY[0] 
  // to the current one BezierControlX[3] BezierControlY[3] 
  ...
  /*
    ...
    for(int i = 0; i < 4; i++) {
      ...
      int indHull = (i + 1) % 4;
      float xA = BezierHull[i].x;
      float yA = BezierHull[i].y;
      float xB = BezierHull[indHull].x;
      float yB = BezierHull[indHull].y;
      while(indHull != 0 && xB == PG_OUT_OF_SCREEN_CURSOR && yB == PG_OUT_OF_SCREEN_CURSOR) {
        indHull = (indHull + 1) % 4;
        xB = BezierHull[indHull].x;
        yB = BezierHull[indHull].y;
      }
      ...
      if(yA != yB) {
        ...
        if (alpha_intersection >= 0 && alpha_intersection <= 1) {
          ...
          if(PixelLocation.y == yA 
            && sign(PixelLocation.x - xA) != sign(PixelLocation.x - xB))  { 
            countIntersections++;
          }
      }
      if(indHull == 0) {
...
*/
/*    if( (sign(PixelLocation.x - BezierHull[0].x) == sign(PixelLocation.x - BezierHull[1].x))
      || (sign(PixelLocation.y - BezierHull[0].y) == sign(PixelLocation.y - BezierHull[3].y)) ) {
      return  0.f; 
    }
*/
...
void main() {
  ...
  repop_density = int(uniform_Update_scenario_var_data[28]);
  ...
  photo_scale_pulse = uniform_Update_scenario_var_data[46];
...
#endif
#if PG_NB_TRACKS == 3
  vec4 trkDecay = vec4(trkDecay_0,trkDecay_1,trkDecay_2,0);
#endif
#if PG_NB_TRACKS == 4
  ...
  */
  ...
  if(photoWeight * uniform_Update_fs_4fv_photo01Wghts_randomValues.x > 0) {
    ...
    coordsImageScaled += uniform_Update_fs_4fv_flashPhotoTrkWght_flashPhotoTrkThres_Photo_offSetsXY.zw;
  ...
  if(photo_satur > 0 ) {
    ...
    photocolor = clamp( powerColor 
      + (photocolor - vec3(powerColor)) * video_satur , 0 , 1 );
  }
  ...
  for( int indTrack = 1 ; indTrack <= PG_NB_TRACKS ; indTrack++ ) {
    ...
    if( graylevel(out_track_FBO[indCurTrack].rgb) > .2 
      ...
      flashToCACumul += uniform_Update_fs_4fv_flashTrkCAWghts[indCurTrack] 
                  * vec4(out_track_FBO[indCurTrack].rgb,1);
    }
  }
  ...
  for(int indTrack = 0 ; indTrack < PG_NB_TRACKS ; indTrack++) {
      if( graylevel(out_track_FBO[indTrack].rgb) > 0 ) {
          out_track_FBO[indTrack].rgb 
               = out_track_FBO[indTrack].rgb - vec3(trkDecay[indTrack]/10,trkDecay[indTrack],trkDecay[indTrack]);
      }
      out_track_FBO[indTrack].rgb 
