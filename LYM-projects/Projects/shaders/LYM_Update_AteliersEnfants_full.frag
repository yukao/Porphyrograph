...
bool	  freeze;
uniform float uniform_Update_scenario_var_data[73];
...
// SOBEL OFFSETS
vec2 offsetsSobel[9] = {vec2( -1, 1 ), vec2( 0, 1 ), vec2( 1, 1 ),
                        vec2( 0, -1 ), vec2( 0, 0 ), vec2( 0, 1 ),
                        vec2( -1, -1 ),vec2( 0, -1 ),vec2( 1, -1 )};
vec2 offsetsSobelPOT[9] = {vec2( -1./2048., 1./2048. ), vec2( 0, 1./2048. ), vec2( 1./2048., 1./2048. ),
                        vec2( 0, -1./2048. ), vec2( 0, 0 ), vec2( 0, 1./2048. ),
                        vec2( -1./2048., -1./2048. ),vec2( 0, -1./2048. ),vec2( 1./2048., -1./2048. )};
...
uniform mat3 fs_homographyForTexture;
...
vec2 multiTypeGenerativeNoise(vec2 texCoordLoc, vec2 usedNeighborOffset) {
  ...
  // PHOTO
    ...
    return texture(uniform_Update_texture_fs_Photo0, coordsImageScaled ).rg;
  }
}
...
float out_gray_drawing( float current_Brush_Radius, int current_brushID ) {
  ...
  /////////////////////////////////////////////////
  // just a point
  if(isBegin && isEnd) { // not really possible since the value of isBeginOrEnd cannot be both <0 and >0
      /////////////////////////////////////////////////
      // WRITING POSITION
  ...
  if( BezierControlX[0] >= 0 && BezierControlY[0] >= 0 
    ...
    // loop on all the points in the subdivision
...
void main() {
  ...
  noiseScale = uniform_Update_scenario_var_data[20];
  noiseType = int(uniform_Update_scenario_var_data[21]);
  noiseLineScale = uniform_Update_scenario_var_data[22];
  noiseAngleScale = uniform_Update_scenario_var_data[23];
  noiseCenterX = uniform_Update_scenario_var_data[24];
  noiseCenterY = uniform_Update_scenario_var_data[25];
  pixel_acc = uniform_Update_scenario_var_data[26];
  pixel_acc_shiftX = uniform_Update_scenario_var_data[27];
  pixel_acc_shiftY = uniform_Update_scenario_var_data[28];
  pixel_radius = uniform_Update_scenario_var_data[29];
  ...
  camera_gamma = uniform_Update_scenario_var_data[39];
  movieWeight = uniform_Update_scenario_var_data[40];
  movieSobel = uniform_Update_scenario_var_data[41];
  invertMovie = (uniform_Update_scenario_var_data[42] > 0 ? true : false);
  invertCamera = (uniform_Update_scenario_var_data[43] > 0 ? true : false);
  video_satur = uniform_Update_scenario_var_data[44];
  video_gamma = uniform_Update_scenario_var_data[45];
  video_value = uniform_Update_scenario_var_data[46];
  video_threshold = uniform_Update_scenario_var_data[47];
  photoWeight = uniform_Update_scenario_var_data[48];
  photo_satur = uniform_Update_scenario_var_data[49];
  photo_gamma = uniform_Update_scenario_var_data[50];
  photo_threshold = uniform_Update_scenario_var_data[51];
  photo_value = uniform_Update_scenario_var_data[52];
  photoSobel = uniform_Update_scenario_var_data[53];
  invertPhoto = (uniform_Update_scenario_var_data[54] > 0 ? true : false);
  photo_scale = uniform_Update_scenario_var_data[55];
  mask_scale = uniform_Update_scenario_var_data[56];
  photo_contrast = uniform_Update_scenario_var_data[57];
  mask_contrast = uniform_Update_scenario_var_data[58];
  CAParams1 = uniform_Update_scenario_var_data[59];
  CAParams2 = uniform_Update_scenario_var_data[60];
  CAParams3 = uniform_Update_scenario_var_data[61];
  CAParams4 = uniform_Update_scenario_var_data[62];
  CAParams5 = uniform_Update_scenario_var_data[63];
  CAParams6 = uniform_Update_scenario_var_data[64];
  CAParams7 = uniform_Update_scenario_var_data[65];
  CAParams8 = uniform_Update_scenario_var_data[66];
  cameraCumul = int(uniform_Update_scenario_var_data[67]);
  CAstep = int(uniform_Update_scenario_var_data[68]);
  CAcolorSpread = (uniform_Update_scenario_var_data[69] > 0 ? true : false);
  master_scale_ratio = uniform_Update_scenario_var_data[70];
  master_offsetY = uniform_Update_scenario_var_data[71];
  ...
  StylusVsRubber = uniform_Update_fs_3fv_frameno_Cursor_flashPartCAWght.y;
...
*/
  ...
  if(photoWeight * uniform_Update_fs_4fv_photo01Wghts_randomValues.y > 0) {
    coordsImage = coordsImage * uniform_Update_fs_4fv_photo01_wh.zw + uniform_Update_fs_4fv_flashPhotoTrkWght_flashPhotoTrkThres_Photo_offSetsXY.zw;
    // coordsImageScaled = coordsImage;
 ...
 */
  ...
  if( cameraSobel > 0 ) {
      ...
      // samples the center pixel and its Moore neighborhood
      for( int i = 0 ; i < 4 ; i++ ) {
          samplerSobel = texture(uniform_Update_texture_fs_Camera_frame , cameraCoord + offsetsSobel[i]).rgb;
          sobelX += sobelMatrixX[i] * samplerSobel;
          sobelY += sobelMatrixY[i] * samplerSobel;
      }
      for( int i = 5 ; i < 9 ; i++ ) {
  ...
  if( movieSobel > 0 ) {
      ...
      // samples the center pixel and its Moore neighborhood
      for( int i = 0 ; i < 4 ; i++ ) {
          samplerSobel = texture(uniform_Update_texture_fs_Movie_frame , movieCoord + offsetsSobel[i]).rgb;
          sobelX += sobelMatrixX[i] * samplerSobel;
          sobelY += sobelMatrixY[i] * samplerSobel;
      }
      for( int i = 5 ; i < 9 ; i++ ) {
  ...
  for( int indTrack = 1 ; indTrack <= PG_NB_TRACKS ; indTrack++ ) {
    ...
    for( int indPath = 0 ; indPath <= PG_NB_PATHS ; indPath++ ) {
      ...
      if( pathStroke > 0 ) {
        ...
        if( StylusVsRubber < 0) { // rubber stylus
    ...
    if( currentPhotoTrack == indCurTrack 
        && flashPhotoTrkWght > 0 
        && graylevel(photoOriginal) > flashPhotoTrkWght ) { // flash photo
  ...
  ///////////////////////////////////////////////////
  ///////////////////////////////////////////////////
  if( pixel_mode == PIXEL_UNIQUE || pixel_mode == PIXEL_FIREWORK ) {
    //////////////////////////////////////
    //////////////////////////////////////
    ...
    // update of the current pixel
      ...
      vec2 pixel_acceleration;
      // FLAT
        pixel_acceleration = multiTypeGenerativeNoise(pixelTextureCoordinatesPOT_XY, vec2(0));      // }

      vec2 acceleration;
      acceleration = pixel_acceleration - pixel_acc_center;
      if( pixel_acc > 0 ) {
      	// updates speed from acceleration
      	out_attachment_FBO[pg_FBO_fs_Pixels_attacht].xy 
          += pixel_acc * acceleration;
      }
      else {
      	// updates speed from damping
      	out_attachment_FBO[pg_FBO_fs_Pixels_attacht].xy 
      ...
      // if the pixel leaves the cell, it is erased (x or y more than 0.5 away from center)
  ...
  else if(pixel_mode == PIXEL_BURST) {
    if(noiseType  >= 0) {
      vec2 initialCoordinates = out_attachment_FBO[pg_FBO_fs_Pixels_attacht].zw;
      vec4 pixelColorBeforeJump = texture( uniform_Update_texture_fs_Trk0, initialCoordinates );
      vec2 rad = normalize(decalCoords - vec2(960,540))/30.0;
      vec2 jump = vec2(0);
      float burstMaskGrayLevel = 0;
      // radial motion according to gray level
      if(noiseType == 0) {
        jump = rad * 1.2 * graylevel(pixelColorBeforeJump.rgb);
      }
      // 
      // radial motion according to discretized mask gray level
      else if(noiseType == 1) {
        jump = rad * 1.2 * floor(graylevel(pixelColorBeforeJump.rgb)*3.) / 3.;
      }
      // horizontal motion according to mask gray level
      else if(noiseType == 2) {
        burstMaskGrayLevel = texture( uniform_Update_texture_fs_Burst_Mask, initialCoordinates ).r;
        jump = vec2(rad.x, 0) * 0.8 * burstMaskGrayLevel;
      }
      // radial motion according to mask gray level
      else if(noiseType == 3) {
        burstMaskGrayLevel = texture( uniform_Update_texture_fs_Burst_Mask, initialCoordinates ).r;
        jump = rad * 0.8 * burstMaskGrayLevel;
      }
      // updates the new position of the current pixel
      out_attachment_FBO[pg_FBO_fs_Pixels_attacht].zw -= jump;
      vec2 currentCoordinates = out_attachment_FBO[pg_FBO_fs_Pixels_attacht].zw;
      float distFromCenter = length(currentCoordinates - vec2(960,540));
      if(distFromCenter < 1) {
        out_track_FBO[0] = vec4( 0, 0, 0, 1 );
      }
      else {
        vec2 fract_coordinates = fract(currentCoordinates);
        if(noiseType == 0 || noiseType == 1) {
          if(min(fract_coordinates.x, fract_coordinates.y) > 0.95 ) {
            out_track_FBO[0] = vec4( 0,0,0, 1 );
          }
          else {
            out_track_FBO[0] = texture( uniform_Update_texture_fs_Trk0, currentCoordinates );
          }
        }
        else if(noiseType == 2 || noiseType == 3) {
          float decimal = fract(burstMaskGrayLevel * 10);
  ...
  if(pixel_mode != PIXEL_BURST) {
    out_track_FBO[0].rgb = clamp(out_track_FBO[0].rgb + flashToBGCumul, 0 , 1);
  }
  else {
    // for pixel burst, a flash on track 0 erases previous value on this track to replace by the flash
    if(graylevel(flashToBGCumul) > 0) {
      out_track_FBO[0].rgb = clamp(flashToBGCumul, 0, 1);
      // resets initial and current position
      out_attachment_FBO[pg_FBO_fs_Pixels_attacht] = vec4(decalCoords,decalCoords);
    }
  ...
  if( graylevel(out_track_FBO[0].rgb) <= 0 ) {
    ...
    // resets pixel initial position and current position if trackO returns to black
    else {
/*      out_attachment_FBO[pg_FBO_fs_Pixels_attacht] = vec4(decalCoords, decalCoords);
*/    }
  }
