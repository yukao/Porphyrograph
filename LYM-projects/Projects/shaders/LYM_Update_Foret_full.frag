...
float	 video_hue;
float	 photo_hue;
uniform float uniform_Update_scenario_var_data[74];
...
float BG_CA_repop_density_weight = 1;
...
vec2 multiTypeGenerativeNoise(vec2 texCoordLoc, vec2 usedNeighborOffset) {
  ...
  // CAMERA
    ...
    return vec2(0); // texture(uniform_Update_texture_fs_Camera_frame, decalCoords + usedNeighborOffset ).rg;
  }
  // MOVIE
  else {
...
void main() {
  ...
  camera_gamma = uniform_Update_scenario_var_data[34];
  cameraThreshold = uniform_Update_scenario_var_data[35];
  cameraWeight = uniform_Update_scenario_var_data[36];
  cameraSobel = uniform_Update_scenario_var_data[37];
  movie_threshold = uniform_Update_scenario_var_data[38];
  ...
  video_value = uniform_Update_scenario_var_data[45];
  movie_gamma = uniform_Update_scenario_var_data[46];
  ...
  photo_value_pulse = uniform_Update_scenario_var_data[51];
  ...
  freeze = (uniform_Update_scenario_var_data[70] > 0 ? true : false);
  video_white = uniform_Update_scenario_var_data[71];
  video_hue = uniform_Update_scenario_var_data[72];
  photo_hue = uniform_Update_scenario_var_data[73];
 ...
 // cursor type (+1 for stylus and -1 for rubber)
  ...
  if( BG_CA_repop_density >= 0 && (repop_CA > 0 || repop_BG > 0)) {
        BG_CA_repop_density_weight = texture(uniform_Update_texture_fs_RepopDensity,decalCoords).r;
  }
 ...
 */
  ...
  // image reading
  cameraOriginal = vec3(0); // texture(uniform_Update_texture_fs_Camera_frame, cameraCoord ).rgb;
...
*/
  ...
  if( CA_on_off && (frameNo % CAstep == 0) ) {
    ...
    // CA "REPOPULATION"
    if( repop_CA > 0 ) {
      if( rand3D(vec3(decalCoordsPOT, uniform_Update_fs_4fv_photo01Wghts_randomValues.z), repop_CA * BG_CA_repop_density_weight) != 0) {
        out4_CA.a = -1.0;
  ...
  // pixel "ADDITION"
  if( repop_BG > 0 ) {
    if( rand3D(vec3(decalCoordsPOT, uniform_Update_fs_4fv_photo01Wghts_randomValues.w), repop_BG * BG_CA_repop_density_weight) != 0) {
