...
-- /transport/startloop/block

add_global_action { 
  pattern = "/transport/startloop/pattern", 
  description = "Enable looping the current pattern",
...
-- /transport/stoploop/block

add_global_action { 
  pattern = "/transport/stoploop/pattern", 
  description = "Disable looping the current pattern",
...
-- for 60bpm + 16 lines, the interval between two lines is 1/16 = 0,0625 (62.5 ms)
...
-- /song/track/XXX/unmute_vol
-- volume between O and 1 is multiplied by db2lin(3) to be scaled to the standard range
-- adds a delay for more random aspect. At 60 bpm with 16 lines, the interval between
-- two lines is 62.5 ms

add_track_action { 
  pattern = "/unmute_vol", 
  description = "Unmute track XXX\nAnd set track XXX's pre FX volume [0 - 1.0]\n"..
    "XXX is the track index, -1 the currently selected track",
  ...
  handler = function(track_index, value)
    ...
    if (track_index >= 1 and track_index <= #tracks) then
      ...
      set_track_parameter(track_index, "prefx_volume", value * 1.4125375446228)
      tracks[track_index].output_delay = math.random( -100 , 100 )
    end
  end,
}
...
-- /song/track/XXX/mute_unmute

add_track_action { 
  pattern = "/mute_unmute", 
  description = "Unmute/mute track XXX\n"..
    "XXX is the track index, -1 the currently selected track",
  ...
  handler = function(track_index)
    local tracks = song().tracks
    if (track_index >= 1 and track_index <= #tracks) then
      if( tracks[track_index].mute_state == renoise.Track.MUTE_STATE_ACTIVE ) then
        tracks[track_index]:mute() 
      else
        tracks[track_index]:unmute() 
      end
