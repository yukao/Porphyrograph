#!/usr/bin/perl
# composition of foreground line drawing and background
# quads

# line drawing based on corrected texture coordinates
# and quad orientation
# can be considered as the basis for any drawing algorithm

use File::stat;
use Time::localtime;

$homedir="/home/christian/Desktop/VR_16";

chdir("$homedir");

system("cmd/PG_confGenerator-v2.prl effe/configuration/LYM_effe-head-single.csv effe/scenarios/LYM_effe-scenario-v2.csv effe/shaders/LYM_Drawing_effe-FS.glsl effe/shaders/LYM_Drawing_effe-FS_full.glsl effe/shaders/LYM_Composition_effe-FS.glsl effe/shaders/LYM_Composition_effe-FS_full.glsl effe/shaders/LYM_TrackDisplay_effe-FS.glsl effe/shaders/LYM_TrackDisplay_effe-FS_full.glsl effe/shaders/LYM_Final_effe-FS.glsl effe/shaders/LYM_Final_effe-FS_full.glsl effe/configuration/LYM_effe.conf");

$dir="pic";
if(-d $dir){
    $date_string = ctime(stat($dir)->mtime);
    $date_string =~ s/^[A-Za-z]+ //; # suppress day name
    $date_string =~ s/:[0-9]+ [A-Za-z ]*/ /; # suppress seconds + locale
    $date_string =~ tr/ /_/;
    $date_string =~ tr/:/_/;

    print "\ndirectory $dir updated at $date_string\n";
    print "mv snapshots/$dir snapshots/${dir}_${date_string}\n";
    system("mv snapshots/$dir snapshots/${dir}_${date_string}");
}
mkdir "snapshots/".$dir or die;

system("nice -19 primusrun ./Porphyrograph/bin/PG_effe effe/configuration/LYM_effe.conf");
