...
float    ReadFloatString( int *p_c  ,
  ...
  resul = (float) ReadIntegerString( p_c , false ,charstring,ind );
  if( *p_c == '.') {
    *p_c = charstring[*ind];
    (*ind)++;
  ...
  if(  *p_c == 'e' ) {
    *p_c = charstring[*ind];
    (*ind)++;
    float exponent = (float)ReadIntegerString( p_c , false ,charstring,ind );
...
void pg_IPClient::sendIPmessages( void ) {
  ...
  while( current_depth_output_stack >= 0 ) {
    ...
    if( message_received
      ...
      // printf( "send message [%s] %d\n" , output_message_stack[ 0 ] , max_network_message_length);
	  ...
	  if( hasPattern ) {
	    ...
	    while( (p_c != (int)EOF) && p_c ) {
	      ...
	      if( next_char_pattern == 'f'
			|| (!next_char_pattern
		      && p_c && ( p_c > 0 && isdigit( p_c) || p_c == '-' ) ) ) {
			  float f = (float) ReadFloatString( &p_c , true ,
		...
		// printf( "argt #%d float %.4f char %c ind %d\n" , nbArg , f , p_c , indArgList);
	      ...
	      // integer: i type in pattern
			  ...
			  int i = (int) ReadIntegerString( &p_c , true ,
							 messageOut , &inChSource );
...
void pg_IPServer::receiveIPMessages( void ) {
  ...
  // #endif
  //   int n = recvfrom(SocketToLocalServer, message, StringLength, 0, 
...
//   }
  ...
  while( n > 0 ){
    ...
    if( receive_format == Plain ) {
      if( strncmp( Input_Message_String + offset_IP_message_received ,
			receive_prefix , 
...
//       }
    ...
    //     n = recvfrom(SocketToLocalServer, message, StringLength, 0, 
    // 		 (struct sockaddr *) &clientAdddress, &clientLength);  
  }
