...
void main()
{
    ...
    if(VertexIn[0].replayTrackLocation.x >= -1) {
        distToReplayTrack = sqrt(length(position0.xy-VertexIn[0].replayTrackLocation));
    }
    ...
    internalRadiusTimesRadiusParameter *= distToReplayTrack;
    ...
    if(partType == MOTEUR) {
        ...
        barycentricWeight
                = clamp(uniform_ParticleSplat_gs_3fv_partRadius_partType_highPitchPulse.z,0,1);
