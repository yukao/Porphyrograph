...
uniform vec4 uniform_Mixing_fs_4fv_pulsedShift_height_msgTrspcy_flashCameraTrkWght;
...
#include_initializations
  ...
  float height = uniform_Mixing_fs_4fv_pulsedShift_height_msgTrspcy_flashCameraTrkWght.y;
  ...
  float flashCameraCoef = uniform_Mixing_fs_4fv_pulsedShift_height_msgTrspcy_flashCameraTrkWght.w;
  ...
  float pulsed_shift = uniform_Mixing_fs_4fv_pulsedShift_height_msgTrspcy_flashCameraTrkWght.x;
  vec4 previous_snapshot_color = texture(fs_lookupTable5, decalCoords + vec2(pulsed_shift,0));
#else
  vec4 previous_snapshot_color = texture(fs_lookupTable5, decalCoords);
#endif
  ...
  if( decalCoords.x < 128.0 && decalCoords.y < height && decalCoords.y > height - 14 
		&& uniform_Mixing_fs_4fv_pulsedShift_height_msgTrspcy_flashCameraTrkWght.z > 0 ) {
	...
	combined_color = mix( combined_color ,  messageColor , 
	   uniform_Mixing_fs_4fv_pulsedShift_height_msgTrspcy_flashCameraTrkWght.z );
  }
