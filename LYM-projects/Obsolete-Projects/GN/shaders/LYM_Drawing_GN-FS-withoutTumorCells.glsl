...
layout (binding = 2) uniform usampler2DRect  fs_lookupTable2;  // data tables for the CA
...
layout (binding = 7) uniform samplerRect  fs_lookupTable7;  // background video texture
...
void main() {
  ...
  if( uniform_Drawing_fs_4fv_trackdecay_CAstep_initCA_webCamOn.w > 0 )  {
    ...
    vec3 videocolor = abs(texture(fs_lookupTable6, videoCoord ).rgb 
                            - texture(fs_lookupTable7, videoCoord ).rgb);
    float gray = graylevel(videocolor);
    if( gray > 0.4 ) {//uniform_Drawing_fs_2fv_mouseTracks_replay_videoThreshold.y ) {
