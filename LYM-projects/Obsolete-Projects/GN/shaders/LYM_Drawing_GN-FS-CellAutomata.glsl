...
uniform vec2 uniform_Drawing_fs_2fv_mouseTracks_replay_videoThreshold;
...
layout (binding = 6) uniform usampler2DRect  fs_lookupTable6;  // data tables for the CA
layout (binding = 7) uniform samplerRect  fs_lookupTable7;  // video texture
layout (binding = 8) uniform samplerRect  fs_lookupTable8;  // video texture
...
void CA_out( vec2 decalCoords , vec4 currentCA ) {
  ...
  if( CAType == CA_GOL || CAType == CA_TOTALISTIC || CAType == CA_GENERATION ) {
    ...
    if( CAType == CA_GENERATION ) { // CA generation number of states + rank (height in data texture)
      CArank = 18;
      nbStates = texelFetch(fs_lookupTable6, 
    ...
    if( currentCA.a < 0 ) {
      out4_CA.a = texture( fs_lookupTable5, 
      ...
      uint newState = 0;
      newState = texelFetch(fs_lookupTable6, 
  ...
  // GENERAL BINARY MOORE NEIGHBORHOOD
    ...
    // uint CArank = 38;
    uint nbStates = texelFetch(fs_lookupTable6, 
    ...
    uint newState = 0;
    if( currentCA.a < 0 ) {
      newState = int(texture( fs_lookupTable5, 
         vec3( particleTextureCoordinates.xy , 
         6.5 / 7.0 ) ).r * (nbStates+1)); // nbStates states randomly
...
*/      out4_CA.rgb = averageSurrounding;
  ...
  // TIME TUNNEL
    ...
    if( CASubType == 1 ) {
      ...
      vec2 noise = texture( fs_lookupTable5, 
			    vec3( particleTextureCoordinates.xy , 
				  6.5 / 7.0 ) ).rg;
  ...
  // WORM
    ...
    // refractory -> ready
    float previousGray =  texture(fs_lookupTable2, decalCoords ).a;
    vec3 noise = texture( fs_lookupTable5, 
			  vec3( particleTextureCoordinates.xy , 
				6.5 / 7.0 ) ).rgb;
    int nbSurroundingFiring =
      (neighborValues[0].a > 0? 1:0) +
...
void main() {
  ...
  if( uniform_Drawing_fs_4fv_trackdecay_CAstep_initCA_webCamOn.w > 0 )  {
    ...
    vec4 videocolor = texture(fs_lookupTable7, videoCoord );
    videocolor.rgb = abs(videocolor.rgb - texture(fs_lookupTable8, videoCoord ).rgb);
    float gray = graylevel(videocolor);
    if( gray > uniform_Drawing_fs_2fv_mouseTracks_replay_videoThreshold.y ) {
      track_0_opacity = gray;
    }
  }
  ...
  if( uniform_Drawing_fs_2fv_mouseTracks_replay_videoThreshold[ 0 ] > 0.0 ) {
  ...
  if( out_CA_FBO.a > 0 ) {
    ...
    // rebuilds output for the gray/drawing buffer after decay
    // has been taken into consideration
    if( graylevel(newCA_w_decay.rgb) > 0 ) {
      out_CA_FBO.rgb = newCA_w_decay.rgb;
    }
    else {
  ...
  if(uniform_Drawing_fs_4fv_trackdecay_CAstep_initCA_webCamOn.z > 0 && abs(decalCoords.x - width/2) < 1.2  && abs(decalCoords.y - height/2) < 1.2 ) {
