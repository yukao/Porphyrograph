...
void updateMouseEnvironmentVariablesAndTables( double theTime ) {
  ...
  if( currentDrawingTrack >= 0
    ...
    ///////////////////////////////////////////////////////////////////////
    if( TrackStatus_source[ currentDrawingTrack ].isActiveRecording 
	&& TrackStatus_source[ currentDrawingTrack ].nbRecordedFrames 
	< max_mouse_recording_frames ) {
      int indFrameRec = TrackStatus_source[ currentDrawingTrack ].nbRecordedFrames;
      // printf( "rec track %d frame %d\n",currentDrawingTrack,indFrameRec );
	
      // records the initial time and the current time to play in synch
      if( indFrameRec == 0 ) {
	TrackStatus_source[ currentDrawingTrack ].initialTimeRecording = theTime;
      }
      TrackTime_source[ currentDrawingTrack ][ indFrameRec ] = theTime;
	
      TrackColor_r_source[ currentDrawingTrack ][ indFrameRec ] 
	= local_color_r
	+ RGBpalette_received[0] * RGBpalette_val_coef;
      TrackColor_g_source[ currentDrawingTrack ][ indFrameRec ]
	= local_color_g
	+ RGBpalette_received[1] * RGBpalette_val_coef;
      TrackColor_b_source[ currentDrawingTrack ][ indFrameRec ]
	= local_color_b
	+ RGBpalette_received[2] * RGBpalette_val_coef;
      TrackColor_a_source[ currentDrawingTrack ][ indFrameRec ]
	= local_color_a;
      ...
      TrackBrushID_source[ currentDrawingTrack ][ indFrameRec ] 
	= brushID;

#ifdef PG_WACOM_TABLET
      TrackRadiusX_source[ currentDrawingTrack ][ indFrameRec ] 
	= brushRadius + brush_radius_vol * brush_radius_vol_coef
      ...
      TrackRadiusY_source[ currentDrawingTrack ][ indFrameRec ] 
	= brushRadius + brush_radius_vol * brush_radius_vol_coef
	  + tabletPressureRadius * brush_radius_pressure_coef
	...
	= brushRadius + brush_radius_vol * brush_radius_vol_coef;
      ...
      TrackPos_x_source[ currentDrawingTrack ][ indFrameRec ] = (float)CurrentMousePos_x;
      TrackPos_y_source[ currentDrawingTrack ][ indFrameRec ] = (float)CurrentMousePos_y;
	...
	float tan_x = Tension * ( TrackPos_x_source[ currentDrawingTrack ][ indFrameRec ] - TrackPos_x_source[ currentDrawingTrack ][ indFrameRec - 2 ]);
	float tan_y = Tension * ( TrackPos_y_source[ currentDrawingTrack ][ indFrameRec ] - TrackPos_y_source[ currentDrawingTrack ][ indFrameRec - 2 ]); 
	// move second step: null left tangent of current point
	if( !wasAMove ) {
	  TrackPos_xL_source[ currentDrawingTrack ][ indFrameRec - 1 ] 
	    = TrackPos_x_source[ currentDrawingTrack ][ indFrameRec - 1 ] - tan_x;
	  TrackPos_yL_source[ currentDrawingTrack ][ indFrameRec - 1 ]  
	    = TrackPos_y_source[ currentDrawingTrack ][ indFrameRec - 1 ] - tan_y;
	}
	else {
	  TrackPos_xL_source[ currentDrawingTrack ][ indFrameRec - 1 ] = 0.0;
	  TrackPos_yL_source[ currentDrawingTrack ][ indFrameRec - 1 ] = 0.0;
	...
	if( !isAMove ) {
	  if( !wasAMove ) {
	    TrackPos_xR_source[ currentDrawingTrack ][ indFrameRec - 1 ] 
	      = TrackPos_x_source[ currentDrawingTrack ][ indFrameRec - 1 ] + tan_x;
	    TrackPos_yR_source[ currentDrawingTrack ][ indFrameRec - 1 ]  
	      = TrackPos_y_source[ currentDrawingTrack ][ indFrameRec - 1 ] + tan_y;
	  }
	  else {
	    // first point in a new curve
	    TrackPos_xR_source[ currentDrawingTrack ][ indFrameRec - 1 ] 
	      = TrackPos_x_source[ currentDrawingTrack ][ indFrameRec - 1 ];
	    TrackPos_yR_source[ currentDrawingTrack ][ indFrameRec - 1 ]  
	      = TrackPos_y_source[ currentDrawingTrack ][ indFrameRec - 1 ];
	  }	      
	}
	else {
	  // assigns null values to right tangent extremity so that it is saved
	  // as a move
	  TrackPos_xR_source[ currentDrawingTrack ][ indFrameRec - 1 ] = 0.0;
	  TrackPos_yR_source[ currentDrawingTrack ][ indFrameRec - 1 ] = 0.0;
	  // a null left tangent extremity will be givent at next frame

	  // null left tangent at closing point before the move
	  TrackPos_xL_source[ currentDrawingTrack ][ indFrameRec - 1 ] = TrackPos_x_source[ currentDrawingTrack ][ indFrameRec - 1 ];
	  TrackPos_yL_source[ currentDrawingTrack ][ indFrameRec - 1 ] = TrackPos_y_source[ currentDrawingTrack ][ indFrameRec - 1 ];
	}

	// right tangent is half way between first point and left tangent extremity of second point
	if( indFrameRec == 2 ) {
	  TrackPos_xR_source[ currentDrawingTrack ][ 0 ] = TrackPos_x_source[ currentDrawingTrack ][ 0 ] 
	    + 0.5F * ( TrackPos_xL_source[ currentDrawingTrack ][ 1 ] - TrackPos_x_source[ currentDrawingTrack ][ 0 ]); 
	  TrackPos_yR_source[ currentDrawingTrack ][ 0 ] = TrackPos_y_source[ currentDrawingTrack ][ 0 ] 
	    + 0.5F * ( TrackPos_yL_source[ currentDrawingTrack ][ 1 ] - TrackPos_y_source[ currentDrawingTrack ][ 0 ]); 
	}
      }	
      // printf( "Track src rec %d Ind %d tan %d %d / prev pos %d %d\n" , currentDrawingTrack + 1 , indFrameRec - 1 , (int)TrackPos_xL_source[ currentDrawingTrack ][ indFrameRec - 1 ] , (int)TrackPos_yL_source[ currentDrawingTrack ][ indFrameRec - 1 ] , (int)TrackPos_xR_source[ currentDrawingTrack ][ indFrameRec - 1 ] , (int)TrackPos_yR_source[ currentDrawingTrack ][ indFrameRec - 1 ] );
    ...
    ///////////////////////////////////////////////////////////////////////
    if( TrackStatus_target[ currentDrawingTrack ].isActiveRecording 
	&& TrackStatus_target[ currentDrawingTrack ].nbRecordedFrames 
	< max_mouse_recording_frames ) {
      int indFrameRec = TrackStatus_target[ currentDrawingTrack ].nbRecordedFrames;
	
      // records the initial time and the current time to play in synch
      if( indFrameRec == 0 ) {
	TrackStatus_target[ currentDrawingTrack ].initialTimeRecording = theTime;
      }
      TrackTime_target[ currentDrawingTrack ][ indFrameRec ] = theTime;
      
      TrackColor_r_target[ currentDrawingTrack ][ indFrameRec ] 
	= local_color_r
	+ RGBpalette_received[0] * RGBpalette_val_coef;
      TrackColor_g_target[ currentDrawingTrack ][ indFrameRec ]
	= local_color_g
	+ RGBpalette_received[1] * RGBpalette_val_coef;
      TrackColor_b_target[ currentDrawingTrack ][ indFrameRec ]
	= local_color_b
	+ RGBpalette_received[2] * RGBpalette_val_coef;
      TrackColor_a_target[ currentDrawingTrack ][ indFrameRec ]
	= local_color_a;
      ...
      TrackBrushID_target[ currentDrawingTrack ][ indFrameRec ] 
	= brushID;
      
#ifdef PG_WACOM_TABLET
      TrackRadiusX_target[ currentDrawingTrack ][ indFrameRec ] 
	= brushRadius+ brush_radius_vol * brush_radius_vol_coef
	  + tabletPressureRadius * brush_radius_pressure_coef
	  + fabs( sin(tabletAzimutRadius) )  * tabletInclinationRadius * brush_radius_angleHor_coef;
      TrackRadiusY_target[ currentDrawingTrack ][ indFrameRec ] 
	= brushRadius+ brush_radius_vol * brush_radius_vol_coef
	  + tabletPressureRadius * brush_radius_pressure_coef
	  + fabs( cos(tabletAzimutRadius) ) * tabletInclinationRadius * brush_radius_angleVer_coef;
#else
	...
	= brushRadius+ brush_radius_vol * brush_radius_vol_coef;
      ...
      TrackPos_x_target[ currentDrawingTrack ][ indFrameRec ] = (float)CurrentMousePos_x;
      TrackPos_y_target[ currentDrawingTrack ][ indFrameRec ] = (float)CurrentMousePos_y;
	...
	// sets tangent extremities to 0 in case of move for not redrawing the
	// jump later & for inserting a move in the saved svg
	float tan_x = Tension * ( TrackPos_x_target[ currentDrawingTrack ][ indFrameRec ] - TrackPos_x_target[ currentDrawingTrack ][ indFrameRec - 2 ]);
	float tan_y = Tension * ( TrackPos_y_target[ currentDrawingTrack ][ indFrameRec ] - TrackPos_y_target[ currentDrawingTrack ][ indFrameRec - 2 ]); 
	// move second step: null left tangent of current point
	if( !wasAMove ) {
	  TrackPos_xL_target[ currentDrawingTrack ][ indFrameRec - 1 ] 
	    = TrackPos_x_target[ currentDrawingTrack ][ indFrameRec - 1 ] - tan_x;
	  TrackPos_yL_target[ currentDrawingTrack ][ indFrameRec - 1 ]  
	    = TrackPos_y_target[ currentDrawingTrack ][ indFrameRec - 1 ] - tan_y;
	}
	else {
	  TrackPos_xL_target[ currentDrawingTrack ][ indFrameRec - 1 ] = 0.0;
	  TrackPos_yL_target[ currentDrawingTrack ][ indFrameRec - 1 ] = 0.0;
	  // a null left tangent was givent at next frame
	}
	// move first step: null right tangent extremity of preceding point
	// so that it is saved as a move
	if( !isAMove ) {
	  if( !wasAMove ) {
	    TrackPos_xR_target[ currentDrawingTrack ][ indFrameRec - 1 ] 
	      = TrackPos_x_target[ currentDrawingTrack ][ indFrameRec - 1 ] + tan_x;
	    TrackPos_yR_target[ currentDrawingTrack ][ indFrameRec - 1 ]  
	      = TrackPos_y_target[ currentDrawingTrack ][ indFrameRec - 1 ] + tan_y;
	  }
	  else {
	    // first point in a new curve
	    TrackPos_xR_target[ currentDrawingTrack ][ indFrameRec - 1 ] 
	      = TrackPos_x_target[ currentDrawingTrack ][ indFrameRec - 1 ];
	    TrackPos_yR_target[ currentDrawingTrack ][ indFrameRec - 1 ]  
	      = TrackPos_y_target[ currentDrawingTrack ][ indFrameRec - 1 ];
	  }	      
	}
	else {
	  // assigns null values to right tangent extremity so that it is saved
	  // as a move
	  TrackPos_xR_target[ currentDrawingTrack ][ indFrameRec - 1 ] = 0.0;
	  TrackPos_yR_target[ currentDrawingTrack ][ indFrameRec - 1 ] = 0.0;
	  // a null left tangent extremity will be givent at next frame
	  
	  // null left tangent at closing point before the move
	  TrackPos_xL_target[ currentDrawingTrack ][ indFrameRec - 1 ] = TrackPos_x_target[ currentDrawingTrack ][ indFrameRec - 1 ];
	  TrackPos_yL_target[ currentDrawingTrack ][ indFrameRec - 1 ] = TrackPos_y_target[ currentDrawingTrack ][ indFrameRec - 1 ];
	}
	// printf( "Track %d Ind %d tan %d %d / prev pos %d %d\n" , ind + 1 , indFrameRec - 1 , (int)TrackPos_xL_target[ currentDrawingTrack ][ indFrameRec - 1 ] , (int)TrackPos_yL_target[ currentDrawingTrack ][ indFrameRec - 1 ] , (int)TrackPos_xR_target[ currentDrawingTrack ][ indFrameRec - 1 ] , (int)TrackPos_yR_target[ currentDrawingTrack ][ indFrameRec - 1 ] );
	// right tangent is half way between first point and left tangent extremity of second point
	if( indFrameRec == 2 ) {
	  TrackPos_xR_target[ currentDrawingTrack ][ 0 ] = TrackPos_x_target[ currentDrawingTrack ][ 0 ] 
	    + 0.5F * ( TrackPos_xL_target[ currentDrawingTrack ][ 1 ] - TrackPos_x_target[ currentDrawingTrack ][ 0 ]); 
	  TrackPos_yR_target[ currentDrawingTrack ][ 0 ] = TrackPos_y_target[ currentDrawingTrack ][ 0 ] 
	    + 0.5F * ( TrackPos_yL_target[ currentDrawingTrack ][ 1 ] - TrackPos_y_target[ currentDrawingTrack ][ 0 ]); 
	}
      }
  ...
  for( int indTrack = 0 ; 
	...
	tracks_Color_r[ indTrack ] = (float)Lerp((float)TrackColor_r_source[ indTrack ][ indFrameReading ] , 
		  ...
		  + RGBpalette_received[0] * RGBpalette_replay_coef;
	tracks_Color_g[ indTrack ] =  (float)Lerp((float)TrackColor_g_source[ indTrack ][ indFrameReading ] , 
		  ...
		  + RGBpalette_received[1] * RGBpalette_replay_coef;
	tracks_Color_b[ indTrack ] =  (float)Lerp((float)TrackColor_b_source[ indTrack ][ indFrameReading ] , 
		  ...
		  + RGBpalette_received[2] * RGBpalette_replay_coef;
  ...
  if( !is_trackreplay ) {
	...
	&& isDrawingOn ) {
      // printf("mouse pos %dx%d %d\n",CurrentMousePos_x,CurrentMousePos_y,currentDrawingTrack);
      tracks_Color_r[ currentDrawingTrack ] = local_color_r
		  + RGBpalette_received[0] * RGBpalette_val_coef;
      tracks_Color_g[ currentDrawingTrack ] = local_color_g
		  + RGBpalette_received[1] * RGBpalette_val_coef;
      tracks_Color_b[ currentDrawingTrack ] = local_color_b
		  + RGBpalette_received[2] * RGBpalette_val_coef;
      tracks_Color_a[ currentDrawingTrack ] = local_color_a
		  + RGBpalette_received[3];
       // printf("Track %d color a %.2f %.2f %.2f %.2f\n" , currentDrawingTrack, local_color_r
	 //+ RGBpalette_received[1] , 
	 //local_color_g 
	 //+ RGBpalette_received[1] , 
	 //local_color_b 
	 //+ RGBpalette_received[2] , 
	 //local_color_a
	 //+ RGBpalette_received[3]  ); 
      tracks_BrushID[ currentDrawingTrack ] = brushID;
#ifdef PG_WACOM_TABLET
      tracks_RadiusX[ currentDrawingTrack ] 
	= brushRadius+ brush_radius_vol * brush_radius_vol_coef
	  + tabletPressureRadius * brush_radius_pressure_coef
	  + fabs( sin(tabletAzimutRadius) )  * tabletInclinationRadius * brush_radius_angleHor_coef;
      tracks_RadiusY[ currentDrawingTrack ]
	= brushRadius+ brush_radius_vol * brush_radius_vol_coef
	  + tabletPressureRadius * brush_radius_pressure_coef
	  + fabs( cos(tabletAzimutRadius) ) * tabletInclinationRadius * brush_radius_angleVer_coef;
  //printf("track %d brushRadius brush_radius_vol brush_radius_vol_coef %.2f %.2f %.2f\n" ,
  //  currentDrawingTrack ,brushRadius, brush_radius_vol , brush_radius_vol_coef );

#else
     tracks_RadiusX[ currentDrawingTrack ] = brushRadius
       + brush_radius_vol * brush_radius_vol_coef;
      tracks_RadiusY[ currentDrawingTrack ] = brushRadius
       + brush_radius_vol * brush_radius_vol_coef;
#endif
      // printf("Track %d brush ID radius %d %.2f\n" , currentDrawingTrack, brushID, brushRadius );
    }
  }
